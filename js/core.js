                ...
                try {
                    ...
                    DB.sales.forEach(s => {
                        if (s.is_manual && s.is_carried_forward) {
                            ...
                            // Unconditionally set timestamp to 1st of current month for carried-forward entries
                    ...
                    if (app.currentUser && app.currentUser.role === 'admin' && !localStorage.getItem('aci_cf_timestamp_db_fixed_v1')) {
                        try {
                            const targetYear = new Date().getFullYear();
                            const monthMap = { "January": "01", "February": "02", "March": "03", "April": "04", "May": "05", "June": "06", "July": "07", "August": "08", "September": "09", "October": "10", "November": "11", "December": "12" };
                            const monthNum = monthMap[app.currentMonth] || "01";
                            ...
                            // Send async update to database without blocking load
                            app.neonSQL([`UPDATE manual_deliveries SET timestamp = '${newTimestamp}' WHERE is_carried_forward = 1 AND timestamp NOT LIKE '%${targetYear}-${monthNum}-01%'`]).then(() => {
                                localStorage.setItem('aci_cf_timestamp_db_fixed_v1', 'true');
                ...
                try {
                    ...
                    DB.sales.forEach(s => {
                        if (s.is_manual && s.is_carried_forward) {
                            ...
                            const monthMap = { "January": "01", "February": "02", "March": "03", "April": "04", "May": "05", "June": "06", "July": "07", "August": "08", "September": "09", "October": "10", "November": "11", "December": "12" };
                            const monthNum = monthMap[app.currentMonth] || "01";
                            if (s.timestamp && new Date(s.timestamp).getMonth() !== (parseInt(monthNum) - 1)) {
                                s.timestamp = `${targetYear}-${monthNum}-01 10:00:00`;
                            }
                        }
                    });
