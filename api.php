...
try {
    ...
    if ($action === 'fix_timestamps') {
        $pdo->exec("UPDATE manual_deliveries SET timestamp = CONCAT('1 ', SUBSTRING(sales_month, 1, 3), ' ', sales_year) WHERE is_carried_forward = 1");
        echo json_encode(['status' => 'ok', 'message' => 'Fixed timestamps']);
        exit;
    }
