        ...
        const DB = {
            users: [
                ...
                { id: 'u3', name: 'Rahim Uddin (SO)', role: 'so', employee_id: 'SO001', territories: ['t1'] },
                { id: 'u4', name: 'Karim Hasan (SO)', role: 'so', employee_id: 'SO002', territories: ['t2'] }
            ],
            territories: [
        ...
        const app = {
            ...
            initNeonDB: async () => {
                try {
                    const module = await import("https://esm.sh/@neondatabase/serverless");
                    const sql = module.neon("postgresql://neondb_owner:npg_5XUZLavKxk9T@ep-solitary-resonance-aoh7grwd-pooler.c-2.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require");
                    app.neonSQL = sql;
                    console.log("Successfully initialized Neon Database connection");

                    // Fetch data concurrently to speed up loading
                    const [targets, projections, emi, sales, recovery_od] = await Promise.all([
                        sql`SELECT * FROM targets`,
                        sql`SELECT * FROM projections`,
                        sql`SELECT * FROM emi`,
                        sql`SELECT * FROM sales`,
                        sql`SELECT * FROM recovery_od`
                    ]);
                    ...
                    console.log("Data successfully loaded from Neon DB.");
                    
                    // Re-render dashboard if user is already logged in and dashboard is visible
                    if (app.currentUser && document.getElementById('dashboard-view') && !document.getElementById('dashboard-view').classList.contains('hidden')) {
                         if (app.currentUser.role === 'so') {
                             if(typeof app.renderSODashboard === 'function') app.renderSODashboard();
                         } else {
                             if(typeof app.renderAdminDashboard === 'function') app.renderAdminDashboard();
                         }
                    }
                } catch (e) {
            ...
            init: () => {
                ...
                app.initNeonDB(); // Connect to Neon DB and fetch data
            ...
            populateLoginDropdown: () => {
                ...
                if (admins.length > 0) {
                    ...
                    admins.forEach(u => optionsHtml += `<option value="${u.id}">Global - ${u.name}</option>`);
                    optionsHtml += `</optgroup>`;
                }

                const ams = DB.users.filter(u => u.role === 'am');
                if (ams.length > 0) {
                    optionsHtml += `<optgroup label="Area Managers">`;
                    ams.forEach(u => {
                        const areaNames = u.territories.map(tId => DB.territories.find(t => t.id === tId)?.name).join(', ') || 'No Area';
                        optionsHtml += `<option value="${u.id}">${areaNames} - ${u.name}</option>`;
                    });
                    optionsHtml += `</optgroup>`;
                }
                ...
                if (sos.length > 0) {
                    optionsHtml += `<optgroup label="Sales Officers">`;
                    sos.forEach(u => {
                        const areaName = DB.territories.find(t => t.id === u.territories[0])?.name || 'Unknown';
                        optionsHtml += `<option value="${u.id}">${areaName} - ${u.name}</option>`;
                    });
                    optionsHtml += `</optgroup>`;
                }
            ...
            handleLogin: (e) => {
                ...
                setTimeout(() => {
                    const user = DB.users.find(u => u.id === userId && u.employee_id === empId);
                    ...
                    if (user) {
                        ...
                        app.showToast('Invalid Employee ID for the selected Area/User.', 'error');
                    }
                }, 1000); // Simulated network request
            },
            ...
            navigateMobile: (target) => {
                localStorage.setItem('aci_last_page', target);
                ...
                // Set active
                const activeEl = document.getElementById(`nav-${target}`);
                if (activeEl) activeEl.classList.add('nav-tab-active', 'text-aci-blue');

                // Route
                switch (target) {
                    case 'dashboard': app.renderAdminDashboard(); break;
                    case 'map': app.renderAdminSalesMap(); break;
                    case 'ai':
                        if (app.currentUser.role === 'admin') app.renderAdminAIInsights();
                        else app.renderAdminDashboard();
                        break;
                    case 'emi': app.renderAdminEMI(); break;
                    case 'notices':
                        if (app.currentUser.role === 'admin') app.renderAdminNotices();
                        else app.renderAdminDashboard();
                        break;
                }
            ...
            setupSidebar: () => {
                ...
                if (app.currentUser.role === 'admin') {
                    links = [
                        ...
                        renderSidebarBtn('settings', 'System Settings', 'settings', 'text-cyan-400', 'app.renderSystemSettings()')
                    ].join('');
                    ...
                    // Hide restricted buttons from mobile bottom nav for AM
                    ['nav-ai', 'nav-notices'].forEach(id => {
                    ...
                    // Hide restricted buttons from mobile bottom nav for SO
                    ['nav-ai', 'nav-notices'].forEach(id => {
                        const btn = document.getElementById(id);
                        if (btn) btn.parentElement.style.display = 'none';
                    });
                    ...
                    <div class="max-w-6xl mx-auto fade-in">
                        <div class="mb-6 flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-sm text-slate-500">Manage brands and model names for SO delivery entry</p>
                            </div>
            ...
            handleAddEditModel: (e, modelId = null) => {
                e.preventDefault();
                    ...
                    <div class="max-w-6xl mx-auto fade-in pb-12">
                        <div class="mb-6 flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-sm text-slate-500">Manage System Administrators, Area Managers, and Sales Officers</p>
                            </div>
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        ${admins.map(u => `
                                            <tr class="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td class="px-6 py-4">
                                                    <span class="bg-rose-100 text-rose-700 px-2 py-1 rounded text-[10px] font-bold uppercase border border-rose-200">${u.role}</span>
                                                </td>
                                                <td class="px-6 py-4">
                        ...
                        <!-- Sales Officer / Territory Section -->
                        <div>
                                                ...
                                                <tr class="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="flex items-center justify-end gap-1">
                                                            ${so ? `
                                                                <button onclick="app.showAddUserModal('${so.id}')" class="text-slate-400 hover:text-aci-blue p-1.5 rounded hover:bg-slate-50 transition-colors tooltip" title="Edit SO"><i data-lucide="edit" class="w-4 h-4"></i></button>
                                                                <button onclick="app.deleteUser('${so.id}')" class="text-slate-400 hover:text-red-500 p-1.5 rounded hover:bg-red-50 transition-colors tooltip" title="Delete SO"><i data-lucide="user-x" class="w-4 h-4"></i></button>
                                                            ` : `
                                                                <button onclick="app.showAddUserModal(null, '${t.id}')" class="text-aci-blue hover:text-indigo-600 font-bold text-[11px] uppercase tracking-wider flex items-center gap-1 px-2 py-1 rounded-md hover:bg-indigo-50 transition-colors">
                                                                    <i data-lucide="user-plus" class="w-3.5 h-3.5"></i> Assign SO
                                                                </button>
                                                            `}
                    ...
                    <div class="bg-white p-6 rounded-[2rem] shadow-2xl relative z-10 w-full max-w-lg mx-4 border border-white">
                        ...
                        <form id="add-user-form" onsubmit="app.handleAddUser(event, ${user ? `'${user.id}'` : 'null'})" class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="new-user-name" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue transition-all" required placeholder="e.g. Shakil Ahmed" value="${user ? user.name.replace(/\s\((AM|SO)\)$/, '') : ''}">
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select id="new-user-role" onchange="app.handleRoleChange()" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue transition-all appearance-none" required>
                                        <option value="so" ${(user && user.role === 'so') || defaultTerritoryId ? 'selected' : ''}>Sales Officer (SO)</option>
                                        <option value="am" ${user && user.role === 'am' ? 'selected' : ''}>Area Manager (AM)</option>
            ...
            handleAddUser: (e, userId = null) => {
                ...
                if (!userId && DB.users.some(u => u.employee_id === empId)) {
                    app.showToast('Employee ID already exists in the system.', 'error');
                    return;
                }
                ...
                const appendedName = name + (role === 'am' ? ' (AM)' : role === 'so' ? ' (SO)' : '');
            ...
            deleteTerritory: (territoryId) => {
                ...
                if (soAssigned) {
                    app.showToast('Cannot delete territory. Unassign the active Sales Officer first.', 'error');
                    return;
                }
            ...
            renderAdminEMI: () => {
                ...
                const paidCust = emiData.filter(e => e.collected >= e.installment).length;
                const unpaidCust = totalCust - paidCust;
                const paidCustPercent = totalCust > 0 ? Math.round((paidCust / totalCust) * 100) : 0;

                const totalInstallment = emiData.reduce((sum, e) => sum + e.installment, 0);
                const totalCol = emiData.reduce((sum, e) => sum + e.collected, 0);
                ...
                const territorySummary = DB.territories.map(t => {
                    ...
                    const payingCust = tEmi.filter(e => e.collected >= e.installment).length;
                    const nonPayingCust = totalCust - payingCust;
                    const tTotalDue = tEmi.reduce((sum, e) => sum + e.installment, 0); // Amount Due (inst)
                    const tAmountCol = tEmi.reduce((sum, e) => sum + e.collected, 0);
                ...
                const html = `
                    <div class="max-w-6xl mx-auto fade-in">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden overflow-x-auto mb-6">
                            ...
                            <table class="w-full text-left text-sm whitespace-nowrap">
                                <thead class="border-b border-slate-200 text-slate-500 uppercase text-xs tracking-wider bg-slate-100/50">
                                    <tr>
                                        <th class="px-6 py-3 font-semibold">Sales Territory</th>
                                        <th class="px-4 py-3 font-semibold text-center">Total Customers</th>
                                        <th class="px-4 py-3 font-semibold text-center text-green-600">Paying Customers</th>
                                        <th class="px-4 py-3 font-semibold text-center text-red-500">Non-Paying Customers</th>
                                        <th class="px-4 py-3 font-semibold text-right">Total Due (Inst)</th>
                                        <th class="px-4 py-3 font-semibold text-right">Amount Collected</th>
                                        <th class="px-6 py-3 font-semibold text-center">Collection Rate %</th>
                                    </tr>
                                    <tr class="bg-slate-50/80">
                                        <th class="px-6 py-2"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Territory..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-4 py-2"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Total Cust..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal text-center shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-4 py-2"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Paying..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal text-center shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-4 py-2"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Unpaid..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal text-center shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-4 py-2"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Due Amt..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal text-right shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-4 py-2"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Collected..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal text-right shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-6 py-2"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Rate..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal text-center shadow-inner placeholder-slate-300 transition-all"></th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    ${territorySummary.map(t => `
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            <td class="px-6 py-3 font-bold text-slate-800">${t.name}</td>
                                            <td class="px-4 py-3 text-center font-semibold text-slate-700">${t.totalCust}</td>
                                            <td class="px-4 py-3 text-center font-bold text-green-600">${t.payingCust}</td>
                                            <td class="px-4 py-3 text-center font-bold text-red-500">${t.nonPayingCust}</td>
                                            <td class="px-4 py-3 text-right font-semibold text-slate-700">${app.formatCurrency(t.tTotalDue)}</td>
                                            <td class="px-4 py-3 text-right font-semibold text-aci-blue">${app.formatCurrency(t.tAmountCol)}</td>
                                            <td class="px-6 py-3 text-center">
                                                <span class="px-2 py-1 rounded text-[10px] font-bold ${t.tColRate >= 80 ? 'bg-green-100 text-green-700 border border-green-200' : (t.tColRate >= 50 ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-red-100 text-red-700 border border-red-200')}">
                                                    ${t.tColRate}%
                                                </span>
                                            </td>
                                        </tr>
                                    `).join('')}
                                    ${territorySummary.length === 0 ? '<tr><td colspan="7" class="px-6 py-6 text-center text-slate-500">No territory data available.</td></tr>' : ''}
                                </tbody>
                            </table>
                        </div>
                            ...
                            <table class="w-full text-left text-sm whitespace-nowrap">
                                <thead class="border-b border-slate-200 text-slate-500 uppercase text-xs tracking-wider">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 font-semibold">Overdue Amt</th>
                                        <th class="px-6 py-4 font-semibold">Collected</th>
                                        <th class="px-6 py-4 font-semibold">Status</th>
                                    </tr>
                                    <tr class="bg-slate-50/80">
                                        <th class="px-6 py-2 border-b border-slate-200"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Customer..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-6 py-2 border-b border-slate-200"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Territory..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-6 py-2 border-b border-slate-200"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Overdue..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-6 py-2 border-b border-slate-200"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Collected..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-6 py-2 border-b border-slate-200"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Status..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    ${emiData.map(e => {
                    const isCleared = e.collected >= e.overdue_total;
                    return `
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4 text-slate-600 text-xs">${DB.territories.find(t => t.id === e.territory_id)?.name || 'Unknown'}</td>
                                            <td class="px-6 py-4 text-red-600 font-semibold">${app.formatCurrency(e.overdue_total)}</td>
                                            <td class="px-6 py-4 text-green-600 font-semibold">${app.formatCurrency(e.collected)}</td>
                                            <td class="px-6 py-4">
                                                ${isCleared
                            ? '<span class="text-green-600 text-xs font-bold flex items-center gap-1 bg-green-50 px-2 py-1 rounded w-max border border-green-100"><i data-lucide="check-circle-2" class="w-3 h-3"></i> Cleared</span>'
                            : '<span class="text-amber-600 text-xs font-bold flex items-center gap-1 bg-amber-50 px-2 py-1 rounded w-max border border-amber-100"><i data-lucide="clock" class="w-3 h-3"></i> Pending</span>'
                        }
                                            </td>
                                        </tr>
                                    `}).join('')}
                                    ${emiData.length === 0 ? '<tr><td colspan="5" class="px-6 py-8 text-center text-slate-500">No EMI data found.</td></tr>' : ''}
                                </tbody>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="max-w-7xl mx-auto fade-in">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden overflow-x-auto">
                            ...
                            <table class="w-full text-left text-sm whitespace-nowrap min-w-[1000px]">
                                <thead class="border-b border-slate-200 text-slate-500 uppercase text-xs tracking-wider bg-white">
                                    <tr>
                                        <th class="px-5 py-3 font-semibold">Territory & Area</th>
                                        <th class="px-5 py-3 font-semibold">Customer Details</th>
                                        <th class="px-5 py-3 font-semibold">Product Info</th>
                                        <th class="px-5 py-3 font-semibold text-right">Financials (BDT)</th>
                                        <th class="px-5 py-3 font-semibold">Offers & Gifts</th>
                                        <th class="px-5 py-3 font-semibold">Logged On</th>
                                    </tr>
                                    <tr class="bg-slate-50/80">
                                        <th class="px-5 py-2 border-b border-slate-200"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Area..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-5 py-2 border-b border-slate-200"><input type="text" onkeyup="app.filterTableGroup(this)" placeholder="Filter Customer..." class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner placeholder-slate-300 transition-all"></th>
                                        <th class="px-5 py-2 border-b border-slate-200">
                                            <select onchange="app.filterTableGroup(this)" class="w-full text-[10px] px-2 py-1.5 rounded border border-slate-200 focus:outline-none focus:border-aci-blue focus:ring-1 focus:ring-aci-blue bg-white font-normal shadow-inner text-slate-600 transition-all cursor-pointer">
                                                <option value="">All Brands</option>
                                                <option value="foton">Foton</option>
                                                <option value="mahindra">Mahindra</option>
                                            </select>
                                        </th>
                                        <th class="px-5 py-2 border-b border-slate-200 text-right"><span class="text-[10px] text-slate-400 font-normal">No filter</span></th>
                                        <th class="px-5 py-2 border-b border-slate-200"><span class="text-[10px] text-slate-400 font-normal">No filter</span></th>
                                        <th class="px-5 py-2 border-b border-slate-200"><span class="text-[10px] text-slate-400 font-normal">Use top filter</span></th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    ${manualSales.map(s => {
                    ...
                    return `
                                        <tr class="hover:bg-slate-50 transition-colors group">
                                            <td class="px-5 py-4 border-b border-slate-100">
                                                ...
                                                <div class="text-[10px] text-slate-500 font-semibold mt-1 flex items-center gap-1">
                                                    <i data-lucide="map-pin" class="w-3 h-3 text-slate-400"></i>
                                                    ${s.upazila || 'N/A'} ${s.district ? `(${s.district})` : ''}
                                                </div>
                                            </td>
                                            <td class="px-5 py-4 border-b border-slate-100">
                                                <div class="font-bold text-slate-800 text-sm truncate max-w-[150px]" title="${s.customer_name || 'N/A'}">${s.customer_name || 'N/A'}</div>
                                                <div class="flex flex-wrap items-center gap-2 mt-1">
                                                    <span class="text-[10px] font-bold text-aci-blue bg-blue-50 px-1.5 py-0.5 rounded">ID: ${s.customer_id}</span>
                                                    ${s.old_customer_id ? `<span class="text-[9px] font-semibold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded" title="Old Customer ID">Old: ${s.old_customer_id}</span>` : ''}
                                                    ${s.purpose_of_use ? `<span class="text-[9px] font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded flex items-center gap-1" title="Purpose of Use"><i data-lucide="briefcase" class="w-3 h-3"></i> ${s.purpose_of_use}</span>` : ''}
                                                </div>
                                            </td>
                                            <td class="px-5 py-4 border-b border-slate-100">
                                                <div class="font-bold text-slate-700 text-sm">${s.model}</div>
                                                <div class="flex items-center gap-1.5 mt-1">
                                                    <img src="${s.brand === 'Foton' ? 'https://i.ibb.co.com/k6Bbdprf/Foton-emblem.png' : 'https://i.ibb.co.com/qLR0vjHR/Mahindra-simbol.png'}" class="h-3 object-contain">
                                                    <span class="text-[10px] text-slate-500 uppercase font-bold tracking-wider">${s.brand}</span>
                                                    <span class="px-1.5 py-0.5 rounded-full text-[9px] font-bold ml-1 ${s.sale_type === 'New Sale' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}">${s.sale_type}</span>
                                                </div>
                                            </td>
                                            <td class="px-5 py-4 border-b border-slate-100 text-right">
                                                <div class="text-sm font-bold text-slate-800">TP: ${app.formatCurrency(s.financials?.tp || 0)}</div>
                                                <div class="text-[11px] text-slate-600 font-medium mt-0.5">DP: ${app.formatCurrency(s.financials?.dp || 0)}</div>
                                                <div class="text-[10px] text-slate-400 font-medium mt-0.5">Tenure: ${s.financials?.tenure || 0} Months</div>
                                            </td>
                                            <td class="px-5 py-4 border-b border-slate-100">
                                                ${s.discounts?.amount > 0 ? `<div class="text-[11px] font-bold text-rose-600 flex items-center gap-1"><i data-lucide="tags" class="w-3 h-3"></i> -${app.formatCurrency(s.discounts.amount)} (${s.discounts.type})</div>` : '<div class="text-[11px] text-slate-400 italic">No Discount</div>'}
                                                ${s.discounts?.gift ? `<div class="text-[10px] text-indigo-600 font-bold mt-1 flex items-center gap-1"><i data-lucide="gift" class="w-3 h-3"></i> ${s.discounts.gift}</div>` : ''}
                                            </td>
                                            <td class="px-5 py-4 border-b border-slate-100 text-xs text-slate-500 font-medium whitespace-nowrap">
                                                <div class="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded w-max">
                                    ...
                                    `}).join('')}
                                    ${manualSales.length === 0 ? '<tr><td colspan="6" class="px-6 py-12 text-center text-slate-500"><div class="flex flex-col items-center gap-3"><i data-lucide="inbox" class="w-8 h-8 text-slate-300"></i><p>No manual deliveries found matching criteria.</p></div></td></tr>' : ''}
                                </tbody>
                            </table>
                        </div>
                    </div>
            ...
            downloadManualCSV: () => {
                ...
                let csv = 'Territory,District,Upazila,Customer ID,Customer Name,Old Customer ID,Purpose of Use,Brand,Model,Sale Type,TP,DP,Tenure,Discount Type,Discount Amount,Gift Item,Logged On\n';
            ...
            renderAdminDashboard: () => {
                ...
                const currFYSales = salesData.filter(s => s.fy === currentFY);
                const lastFYSales = salesData.filter(s => s.fy === lastFY);
                ...
                // Area (AM) Aggregation
                const amMapping = {
                    't1': 'Md. Shafiqul', 't2': 'Md. Shafiqul',
                    't3': 'Karim Uddin', 't6': 'Karim Uddin',
                    't4': 'Hasan Mahmud', 't8': 'Hasan Mahmud',
                    't5': 'SM Akram', 't7': 'SM Akram'
                };

                const areaStatsMap = {};
                activeTerritories.forEach(t => {
                    const amName = amMapping[t.id] || 'General Area';
                    const amUser = DB.users.find(u => u.role === 'am' && (u.name.includes(amName) || amName.includes(u.name.split(' (')[0])));
                    const areaName = amUser?.area_name || 'Unassigned Area';

                    if (!areaStatsMap[amName]) {
                        areaStatsMap[amName] = {
                            name: amName,
                            areaName: areaName,
                    ...
                    const perf = app.getPerformance(t.id, brandFilter, currentSaleType);
                    areaStatsMap[amName].ytd.budget += perf.ytd.budget;
                    areaStatsMap[amName].ytd.sales += perf.ytd.sales;
                    areaStatsMap[amName].ytd.sply += perf.ytd.sply;
                    areaStatsMap[amName].lastMonth.budget += perf.lastMonth.budget;
                    areaStatsMap[amName].lastMonth.sales += perf.lastMonth.sales;
                    areaStatsMap[amName].lastMonth.sply += perf.lastMonth.sply;

                    const tTargets = DB.targets.filter(tg => tg.territory_id === t.id && tg.brand === brandFilter && tg.sale_type === currentSaleType);
                    ...
                    areaStatsMap[amName].currBudget += tBudgetStats;

                    const tProjs = DB.projections.filter(p => p.territory_id === t.id && p.brand === brandFilter && p.month === app.currentMonth && p.sale_type === currentSaleType);
                    areaStatsMap[amName].currProj += tProjs.reduce((sum, p) => sum + Number(p.projection_qty || 0), 0);
                    ...
                    areaStatsMap[amName].currSales += tSales;
                    tSalesRecords.forEach(s => { areaStatsMap[amName].modelSales[s.model] = (areaStatsMap[amName].modelSales[s.model] || 0) + Number(s.unit_qty || 0); });
                    ...
                    areaStatsMap[amName].mockCurrSply += tCurrSply;
                });
                ...
                const html = `
                    <div class="fade-in pb-12">
                        ...
                        <div class="mb-4">
                                    ...
                                    <div class="flex flex-wrap items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                                        ...
                                        <div class="flex items-center gap-2 flex-grow sm:flex-grow-0 justify-between sm:justify-start">
                                            ...
                                            <div class="flex bg-black/20 p-1 rounded-xl border border-white/10 backdrop-blur-md">
                                                <button onclick="app.adminBrandTab='Foton'; app.renderAdminDashboard()" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${brandFilter === 'Foton' ? 'bg-white shadow-lg text-aci-blue scale-105' : 'text-white/40 hover:text-white/70'}">
                                                ...
                                                <button onclick="app.adminBrandTab='Mahindra'; app.renderAdminDashboard()" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all ${brandFilter === 'Mahindra' ? 'bg-white shadow-lg text-aci-blue scale-105' : 'text-white/40 hover:text-white/70'}">
                            ...
                            <!-- AM Style SO-Dashboard Blocks -->
                            ${(() => {
                             const ytdMonths = app.getYtdMonths(app.currentMonth);
                             const passedMonths = ytdMonths.length;
                             const monthlyTargets = activeTgts.filter(tg => ytdMonths.includes(tg.month));
                             const ytdTargetTillLastMonth = monthlyTargets.length > 0 ? monthlyTargets.reduce((sum, tg) => sum + Number(tg.target_qty || 0), 0) : Math.round((totalYearlyTarget / 12) * passedMonths);
                             const tillLastMonthSalesUnits = currFYSales.filter(s => s.brand === brandFilter && ytdMonths.includes(s.sales_month)).reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);

                             const totalMonthlyProjection = DB.projections.filter(p => activeTerritories.some(t => t.id === p.territory_id) && p.brand === brandFilter && p.month === app.currentMonth && p.sale_type === currentSaleType).reduce((sum, p) => sum + Number(p.projection_qty || 0), 0);
                                 ...
                                 <div class="glass p-4 rounded-xl shadow-sm border border-white/60 mb-4 relative overflow-hidden">
                                     ...
                                     <div class="flex justify-between items-center mb-3">
                                         <h3 class="font-bold text-slate-800 text-sm flex items-center gap-2">
                                             ...
                                             YTD Overall Area (2025-26)
                                         </h3>
                                         <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Till ${app.lastMonth}</span>
                                     </div>
                                    <div class="grid grid-cols-6 text-center divide-x divide-slate-100 mb-4">
                                        <div class="px-1 flex flex-col justify-center">
                                            <p class="text-[9px] text-slate-400 uppercase font-semibold">YTD Target</p>
                                    ...
                                    <div class="mt-4 border-t border-slate-100 pt-3">
                                        ...
                                        <div class="space-y-1">
                                            ${activeTerritories.map(t => {
                                const tTgtObj = DB.targets.filter(tg => tg.territory_id === t.id && tg.brand === brandFilter && tg.sale_type === currentSaleType);
                                const tBudgetYTD = Math.round((tTgtObj.reduce((sum, tg) => sum + Number(tg.target_qty || 0), 0) / 12) * passedMonths);
                                const tSalesYTD = currFYSales.filter(s => s.territory_id === t.id && s.brand === brandFilter && ytdMonths.includes(s.sales_month)).reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);
                            ...
                            <!-- Area Manager: Current Month Performance Dashboard (Creative pacing) -->
                            ${(() => {
                            ...
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                                ...
                                <div class="flex items-center gap-2 w-full sm:w-auto">
                                    <button onclick="app.renderSystemSettings()" class="flex items-center gap-2 px-3.5 py-2 bg-gradient-to-r from-slate-700 to-slate-900 text-white rounded-xl shadow-md hover:shadow-lg hover:scale-[1.03] active:scale-95 transition-all duration-300 font-black text-[10px] uppercase tracking-wider group relative overflow-hidden">
                                        <span class="absolute inset-0 w-full h-full bg-white/10 transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-out"></span>
                                        <i data-lucide="settings" class="w-3.5 h-3.5 transition-transform group-hover:rotate-90"></i>
                    ...
                    <div class="glass p-5 rounded-[2rem] border border-white shadow-xl mb-8 relative overflow-hidden">
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 relative z-10 gap-4">
                            <div>
                    ...
                    ${(() => {
                        ...
                        let gTot = { fyBgt: 0, yBgt: 0, yAct: 0, ySht: 0, lBgt: 0, lAct: 0, cBgt: 0, cTM3: 0, cAum: 0, cTun: 0, cTot: 0 };
                        ...
                        brands.forEach(b => {
                            ...
                            let bProjs = DB.projections.filter(p => p.fy === currentFY && p.brand === b && p.sale_type === currentSaleType && p.month === currMonth);
                            ...
                            if (isAM) {
                                ...
                                bProjs = bProjs.filter(p => app.currentUser.territories.includes(p.territory_id));
                            }
                            if (app.adminTerritoryFilter && app.adminTerritoryFilter !== 'All') {
                                ...
                                bProjs = bProjs.filter(p => p.territory_id === app.adminTerritoryFilter);
                            }

                            const fyBgt = bTgts.reduce((sum, t) => sum + Number(t.target_qty || 0), 0);
                            const yBgt = bTgts.filter(t => ytdMonths.includes(t.month)).reduce((sum, t) => sum + Number(t.target_qty || 0), 0) || Math.round((fyBgt / 12) * ytdMonths.length);
                            const yAct = bSales.filter(s => ytdMonths.includes(s.sales_month)).reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);
                            ...
                            const cSales = bSales.filter(s => s.sales_month === currMonth);
                            const cTM3 = b === 'Foton' ? cSales.filter(s => s.model === 'TM3').reduce((sum, s) => sum + Number(s.unit_qty || 0), 0) : 0;
                            const cAum = b === 'Foton' ? cSales.filter(s => s.model === 'Auman').reduce((sum, s) => sum + Number(s.unit_qty || 0), 0) : 0;
                            const cTun = b === 'Foton' ? cSales.filter(s => s.model === 'Tunland').reduce((sum, s) => sum + Number(s.unit_qty || 0), 0) : 0;
                            
                            let cTot = bProjs.reduce((sum, p) => sum + Number(p.projection_qty || 0), 0);
                            if (cTot === 0) cTot = cSales.reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);

                            const cAch = cBgt > 0 ? Math.round((cTot/cBgt)*100) : 0;

                            gTot.fyBgt += fyBgt; gTot.yBgt += yBgt; gTot.yAct += yAct; gTot.ySht += ySht;
                            gTot.lBgt += lBgt; gTot.lAct += lAct; gTot.cBgt += cBgt;
                            gTot.cTM3 += cTM3; gTot.cAum += cAum; gTot.cTun += cTun; gTot.cTot += cTot;
                            ...
                            brandSummaryRowsHTML += `
                                <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors text-center">
                                    ...
                                    <td class="px-3 py-2 font-medium text-slate-500">${yBgt}</td>
                                    <td class="px-3 py-2 font-bold text-slate-800">${yAct}</td>
                                    <td class="px-3 py-2 font-black ${achColor(yAch)}">${yAch}%</td>
                                    <td class="px-3 py-2 font-bold text-rose-500 border-r border-slate-100">${ySht}</td>

                                    <td class="px-3 py-2 font-medium text-slate-500 bg-slate-50/30">${lBgt}</td>
                                    <td class="px-3 py-2 font-bold text-slate-800 bg-slate-50/30">${lAct}</td>
                                    <td class="px-3 py-2 font-black ${achColor(lAch)} bg-slate-50/30 border-r border-slate-100">${lAch}%</td>

                                    <td class="px-3 py-2 font-medium text-slate-500">${cBgt}</td>
                                    <td class="px-3 py-2 font-bold text-indigo-600 bg-indigo-50/30">${cTM3 || '-'}</td>
                                    <td class="px-3 py-2 font-bold text-indigo-600 bg-indigo-50/30">${cAum || '-'}</td>
                                    <td class="px-3 py-2 font-bold text-indigo-600 bg-indigo-50/30">${cTun || '-'}</td>
                                    <td class="px-3 py-2 font-black text-indigo-900 bg-indigo-100/50">${cTot}</td>
                        ...
                        const gCAch = gTot.cBgt > 0 ? Math.round((gTot.cTot/gTot.cBgt)*100) : 0;
                        const achColor = (a) => a >= 100 ? 'text-emerald-600' : (a >= 80 ? 'text-amber-500' : 'text-rose-500');
                                    ...
                                    <table class="w-full text-left text-[11px] whitespace-nowrap border-collapse">
                                        <thead>
                                            <tr class="bg-slate-100/80 text-slate-600 uppercase tracking-widest text-[9px] border-b border-slate-200">
                                                ...
                                                <th class="px-3 py-2 text-center bg-violet-500/10 text-violet-800 border-r border-slate-200 font-black" colspan="4">YTD (${ytdMonths.length > 0 ? `Jul-${lastMonth.substring(0,3)}` : 'N/A'})</th>
                                                <th class="px-3 py-2 text-center bg-amber-500/10 text-amber-800 border-r border-slate-200 font-black" colspan="3">Last Month (${lastMonth.substring(0,3)})</th>
                                                <th class="px-3 py-2 text-center bg-emerald-500/10 text-emerald-800 font-black" colspan="6">Current Month (${currMonth.substring(0,3)})</th>
                                            </tr>
                                            <tr class="bg-slate-50/80 text-slate-500 uppercase tracking-widest text-[9px] border-b border-slate-200 text-center">
                                                ...
                                                <!-- Current Month -->
                                                <th class="px-3 py-1.5 border-t border-slate-200">Budget</th>
                                                <th class="px-3 py-1.5 border-t border-indigo-100 bg-indigo-50/50 text-indigo-700">TM3</th>
                                                <th class="px-3 py-1.5 border-t border-indigo-100 bg-indigo-50/50 text-indigo-700">Auman</th>
                                                <th class="px-3 py-1.5 border-t border-indigo-100 bg-indigo-50/50 text-indigo-700">Tunland</th>
                                                <th class="px-3 py-1.5 border-t border-indigo-200 bg-indigo-100 text-indigo-900 font-extrabold">Total Proj</th>
                                                <th class="px-3 py-1.5 border-t border-slate-200">Ach%</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr class="bg-slate-100/50 font-black text-slate-800 text-center border-t-2 border-slate-200">
                                                ...
                                                <td class="px-3 py-2">${gTot.yBgt}</td>
                                                <td class="px-3 py-2">${gTot.yAct}</td>
                                                <td class="px-3 py-2 ${achColor(gYAch)}">${gYAch}%</td>
                                                <td class="px-3 py-2 text-rose-600 border-r border-slate-200">${gTot.ySht}</td>

                                                <td class="px-3 py-2 bg-slate-50/30">${gTot.lBgt}</td>
                                                <td class="px-3 py-2 bg-slate-50/30">${gTot.lAct}</td>
                                                <td class="px-3 py-2 ${achColor(gLAch)} bg-slate-50/30 border-r border-slate-200">${gLAch}%</td>

                                                <td class="px-3 py-2">${gTot.cBgt}</td>
                                                <td class="px-3 py-2 text-indigo-700 bg-indigo-50/50">${gTot.cTM3 || '-'}</td>
                                                <td class="px-3 py-2 text-indigo-700 bg-indigo-50/50">${gTot.cAum || '-'}</td>
                                                <td class="px-3 py-2 text-indigo-700 bg-indigo-50/50">${gTot.cTun || '-'}</td>
                                                <td class="px-3 py-2 text-indigo-900 bg-indigo-200/50 font-extrabold">${gTot.cTot}</td>
                    ...
                    <!-- Area Manager: Territory Performance Analytics Table (Admin Parity) -->
                    ${isAM ? `
                    <div class="glass rounded-[2rem] border border-white shadow-xl overflow-hidden mb-8 relative">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-[11px] whitespace-nowrap">
                                <thead>
                                    <tr class="bg-slate-50/80 text-slate-500 uppercase tracking-widest text-[9px] border-b border-slate-100">
                                        <th class="px-6 py-4 font-black sticky left-0 z-10 bg-slate-50">Territory</th>
                                        ...
                                        <th class="px-3 py-4 text-center border-l border-slate-100" colspan="${4 + activeModels.length}">Current Month (${app.currentMonth.substring(0, 3)})</th>
                                    </tr>
                                    <tr class="text-slate-400 uppercase tracking-tighter text-[9px] border-b border-slate-100 text-center">
                                        <th class="px-6 py-3 sticky left-0 z-10 bg-white"></th>
                                        ${app.adminShowYTD ? `
                                        ...
                                        ${activeModels.map(m => `<th class="px-2 py-3 bg-slate-50">${m}</th>`).join('')}
                            ...
                            const rowsHTML = activeTerritories.filter(t => !app.adminTerritoryFilter || app.adminTerritoryFilter === 'All' || t.id === app.adminTerritoryFilter).map(t => {
                                const perf = app.getPerformance(t.id, brandFilter, currentSaleType);
                                const tTargets = DB.targets.filter(tg => tg.territory_id === t.id && tg.brand === brandFilter && tg.sale_type === currentSaleType);
                                const currTgts = tTargets.filter(tg => tg.month === app.currentMonth);
                                ...
                                return `
                                                <tr class="hover:bg-slate-50/50 transition-colors group text-center border-b border-slate-100/50">
                                                    <td class="px-6 py-1.5 text-left sticky left-0 z-10 bg-white border-r border-slate-50 shadow-[4px_0_10px_-5px_rgba(0,0,0,0.05)]">
                                                        <div class="flex items-center gap-2">
                                                            <div class="w-1.5 h-4.5 bg-${h}-500 rounded-full"></div>
                                                            <span class="font-black text-slate-700">${t.name}</span>
                                                        </div>
                                                    </td>
                                                    ${app.adminShowYTD ? `
                                                        <td class="px-2 py-1.5 text-slate-400 font-medium text-center">${perf.ytd.budget}</td>
                                                        <td class="px-2 py-1.5 font-black text-slate-700 text-center">${perf.ytd.sales}</td>
                                                        <td class="px-2 py-1.5 text-center"><span class="px-1.5 py-0.5 rounded-lg bg-${h}-50 text-${h}-600 font-black">${ytdAchVal}%</span></td>
                                                        <td class="px-2 py-1.5 font-bold text-rose-500 text-center">${Math.max(0, perf.ytd.budget - perf.ytd.sales)}</td>
                                                    ` : ''}
                                                    ${app.adminShowLastMonth ? `
                                                        <td class="px-2 py-1.5 text-slate-400 font-medium text-center">${perf.lastMonth.budget}</td>
                                                        <td class="px-2 py-1.5 font-black text-slate-700 text-center">${perf.lastMonth.sales}</td>
                                                        <td class="px-2 py-1.5 font-black text-slate-600 text-center">${ach(perf.lastMonth.sales, perf.lastMonth.budget)}%</td>
                                                    ` : ''}
                                                    <td class="px-2 py-1.5 bg-slate-50/50 text-slate-400 font-medium text-center">${currBudget}</td>
                                                    <td class="px-2 py-1.5 bg-slate-50/50 font-black text-slate-700 text-center">${currProj}</td>
                                                    ${activeModels.map(m => `<td class="px-2 py-1.5 bg-slate-50/50 font-bold text-center ${modelMap[m] ? 'text-indigo-600' : 'text-slate-300'}">${modelMap[m] || '-'}</td>`).join('')}
                                                    <td class="px-2 py-1.5 bg-indigo-50/30 font-black text-indigo-700 text-sm text-center">${currSalesUnits}</td>
                                                    <td class="px-2 py-1.5 bg-indigo-50/30 font-black text-emerald-600 text-center">${ach(currSalesUnits, currBudget)}%</td>
                                                </tr>
                                            `;
                            }).join('');
                            ...
                            const totalRowHTML = `
                                            <tr class="bg-indigo-50/20 font-black text-slate-800 text-center border-t border-indigo-100/55">
                                                <td class="px-6 py-2 text-left sticky left-0 z-10 bg-indigo-50 border-r border-indigo-100 shadow-[4px_0_10px_-5px_rgba(0,0,0,0.05)]">
                                                    <div class="flex items-center gap-2">
                                                ...
                                                ${app.adminShowYTD ? `
                                                    <td class="px-2 py-2 text-slate-700 font-extrabold bg-indigo-50/10 text-center">${totalYtdBudget}</td>
                                                    <td class="px-2 py-2 font-black text-slate-900 bg-indigo-50/10 text-center">${totalYtdSales}</td>
                                                    <td class="px-2 py-2 bg-indigo-50/10 text-center"><span class="px-1.5 py-0.5 rounded-lg bg-${totalH}-50 text-${totalH}-600 font-black">${totalYtdAchVal}%</span></td>
                                                    <td class="px-2 py-2 font-bold text-rose-600 bg-indigo-50/10 text-center">${totalYtdShort}</td>
                                                ` : ''}
                                                ${app.adminShowLastMonth ? `
                                                    <td class="px-2 py-2 text-slate-700 font-extrabold text-center">${totalLastMonthBudget}</td>
                                                    <td class="px-2 py-2 font-black text-slate-900 text-center">${totalLastMonthSales}</td>
                                                    <td class="px-2 py-2 font-black text-slate-800 text-center">${ach(totalLastMonthSales, totalLastMonthBudget)}%</td>
                                                ` : ''}
                                                <td class="px-2 py-2 bg-slate-100/50 text-slate-700 font-extrabold text-center">${totalCurrBudget}</td>
                                                ...
                                                <td class="px-2 py-2 bg-indigo-100/50 font-black text-indigo-700 text-sm text-center">${totalCurrSalesUnits}</td>
                                                <td class="px-2 py-2 bg-indigo-100/50 font-black text-emerald-600 text-center">${ach(totalCurrSalesUnits, totalCurrBudget)}%</td>
                                            </tr>
                                        `;
                        ...
                        <div class="glass rounded-[2rem] border border-white shadow-xl overflow-hidden">
                                ...
                                <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button onclick="app.pulseDetailedView = !app.pulseDetailedView; app.renderAdminDashboard()" 
                                                class="shrink-0 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-1.5 ${app.pulseDetailedView ? 'bg-gradient-to-r from-violet-600 to-indigo-600 text-white shadow-lg shadow-indigo-200 border border-indigo-500/20' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50'}">
                                            <i data-lucide="${app.pulseDetailedView ? 'layout-grid' : 'list'}" class="w-3.5 h-3.5"></i>
                                            Detailed View
                                        </button>
                                        <select onchange="app.adminBrandTab=this.value; app.renderAdminDashboard()" class="bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-slate-700 focus:outline-none focus:border-indigo-500 shadow-sm">
                                ...
                                <table class="w-full text-left text-[11px] whitespace-nowrap border-collapse">
                                    <thead>
                                        ...
                                        <tr class="bg-slate-50/80 text-slate-500 uppercase tracking-widest text-[9px] border-b border-slate-200">
                                            <th class="px-6 py-2 font-black sticky left-0 z-10 bg-slate-50 border-r border-slate-200/80 shadow-[2px_0_5px_rgba(0,0,0,0.02)]" rowspan="3">
                                            ...
                                            <th class="px-3 py-2 text-center bg-gradient-to-r from-violet-600/10 via-violet-500/5 to-transparent text-violet-700 border-l-2 border-r-2 border-t border-violet-500/20 font-extrabold shadow-sm" colspan="20">Q1 (July - September)</th>
                                            <th class="px-3 py-2 text-center bg-gradient-to-r from-amber-600/10 via-amber-500/5 to-transparent text-amber-700 border-l-2 border-r-2 border-t border-amber-500/20 font-extrabold shadow-sm" colspan="20">Q2 (October - December)</th>
                                            <th class="px-3 py-2 text-center bg-gradient-to-r from-emerald-600/10 via-emerald-500/5 to-transparent text-emerald-700 border-l-2 border-r-2 border-t border-emerald-500/20 font-extrabold shadow-sm" colspan="20">Q3 (January - March)</th>
                                            <th class="px-3 py-2 text-center bg-gradient-to-r from-cyan-600/10 via-cyan-500/5 to-transparent text-cyan-700 border-l-2 border-r-2 border-t border-cyan-500/20 font-extrabold shadow-sm" colspan="20">Q4 (April - June)</th>
                                        </tr>
                                        ...
                                        <tr class="bg-slate-50/40 text-slate-500 uppercase tracking-widest text-[9px] border-b border-slate-200 text-center">
                                            <!-- Q1 Months -->
                                            <th class="px-2 py-1.5 border-l-2 border-violet-200/60" colspan="5">July</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">August</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">September</th>
                                            <th class="px-2 py-1.5 bg-violet-500/10 text-violet-800 font-extrabold border-l-2 border-violet-300" colspan="5">Q1 Total</th>
                                            <!-- Q2 Months -->
                                            <th class="px-2 py-1.5 border-l-2 border-amber-200/60" colspan="5">October</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">November</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">December</th>
                                            <th class="px-2 py-1.5 bg-amber-500/10 text-amber-800 font-extrabold border-l-2 border-amber-300" colspan="5">Q2 Total</th>
                                            <!-- Q3 Months -->
                                            <th class="px-2 py-1.5 border-l-2 border-emerald-200/60" colspan="5">January</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">February</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">March</th>
                                            <th class="px-2 py-1.5 bg-emerald-500/10 text-emerald-800 font-extrabold border-l-2 border-emerald-300" colspan="5">Q3 Total</th>
                                            <!-- Q4 Months -->
                                            <th class="px-2 py-1.5 border-l-2 border-cyan-200/60" colspan="5">April</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">May</th>
                                            <th class="px-2 py-1.5 border-l border-slate-200" colspan="5">June</th>
                                            <th class="px-2 py-1.5 bg-cyan-500/10 text-cyan-800 font-extrabold border-l-2 border-cyan-300" colspan="5">Q4 Total</th>
                                        </tr>
                                        <!-- Row 3: Metrics -->
                                            ...
                                            ${(() => {
                                                ...
                                                for (let i = 0; i < 16; i++) {
                                                    ...
                                                    result += metrics.map((m, mIdx) => `
                                                        <th class="px-1.5 py-1 ${mIdx === 0 ? qBorder : ''} ${isQuarterTotal ? 'bg-slate-100/30 font-extrabold' : ''} text-center">${m}</th>
                                                    `).join('');
                                                }
                                                return result;
                                            })()}
                                        ...
                                        ${(() => {
                                            ...
                                            const detailedRowsHTML = mappedTerritories.map((mt, idx) => {
                                                ...
                                                Object.entries(quarters).forEach(([qName, qMonths]) => {
                                                    qMonths.forEach(m => {
                                                        ...
                                                        cellsHTML += `
                                                            <td class="px-2 py-1.5 text-slate-400 font-medium border-l border-slate-100">${p.budget}</td>
                                                            <td class="px-2 py-1.5 font-bold text-slate-700">${p.sales}</td>
                                                            <td class="px-2 py-1.5 font-black text-slate-800">${p.ach}%</td>
                                                            <td class="px-2 py-1.5 text-slate-400 font-medium">${p.sply}</td>
                                                            <td class="px-2 py-1.5 font-black text-[10px]">${formatDetailedGrw(p.growth)}</td>
                                                        `;
                                                    });
                                                    ...
                                                    cellsHTML += `
                                                        <td class="px-2 py-1.5 font-bold ${qBg} ${qText} border-l-2 border-slate-200">${q.budget}</td>
                                                        <td class="px-2 py-1.5 font-black ${qBg} ${qText}">${q.sales}</td>
                                                        <td class="px-2 py-1.5 font-black ${qBg}"><span class="px-1.5 py-0.5 rounded-lg ${qPill} font-black">${q.ach}%</span></td>
                                                        <td class="px-2 py-1.5 font-bold ${qBg} ${qText}">${q.sply}</td>
                                                        <td class="px-2 py-1.5 font-black ${qBg} text-[10px] border-r border-slate-200">${formatDetailedGrw(q.growth)}</td>
                                                    `;
                                                });

                                                return `
                                                    <tr class="pulse-tr-premium text-center border-b border-slate-100 group">
                                                        <td class="px-6 py-1.5 text-left sticky left-0 z-10 bg-white border-r-2 border-slate-200/90 shadow-[2px_0_5px_rgba(0,0,0,0.02)] sticky-left">
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <span class="font-black text-slate-700">${mt.name}</span>
                                                            </div>
                                                        </td>
                                                        <td class="px-3 py-1.5 font-bold text-amber-800 bg-amber-500/[0.01] border-l-2 border-r-2 border-amber-500/10 text-center"><span class="px-2 py-0.5 rounded-lg text-amber-800 font-extrabold pulse-badge-amber text-[10px]">${totalFYBudget}</span></td>
                                                        ${cellsHTML}
                                                    </tr>
                                                `;
                                            }).join('');
                                            ...
                                            Object.entries(quarters).forEach(([qName, qMonths]) => {
                                                qMonths.forEach(m => {
                                                    ...
                                                    grandCellsHTML += `
                                                        <td class="px-2 py-2 text-slate-700 font-extrabold border-l border-slate-200/50 bg-slate-50/50">${g.budget}</td>
                                                        <td class="px-2 py-2 font-black text-slate-900 bg-slate-50/50">${g.sales}</td>
                                                        <td class="px-2 py-2 bg-slate-50/50 font-black text-slate-800">${gAch}%</td>
                                                        <td class="px-2 py-2 text-slate-700 font-extrabold bg-slate-50/50">${g.sply}</td>
                                                        <td class="px-2 py-2 font-black text-[10px] bg-slate-50/50">${formatDetailedGrw(gGrw)}</td>
                                                    `;
                                                });
                                                ...
                                                grandCellsHTML += `
                                                    <td class="px-2 py-2 font-extrabold ${qBg} ${qText} border-l-2 border-slate-300 shadow-inner">${gq.budget}</td>
                                                    <td class="px-2 py-2 font-black ${qBg} ${qText} shadow-inner">${gq.sales}</td>
                                                    <td class="px-2 py-2 ${qBg} shadow-inner"><span class="px-1.5 py-0.5 rounded-lg ${pillBg} font-black">${gqAch}%</span></td>
                                                    <td class="px-2 py-2 font-extrabold ${qBg} ${qText} shadow-inner">${gq.sply}</td>
                                                    <td class="px-2 py-2 font-black ${qBg} text-[10px] border-r border-slate-300 shadow-inner">${formatDetailedGrw(gqGrw)}</td>
                                                `;
                                            });

                                            const grandTotalRowHTML = `
                                                <tr class="bg-indigo-50/20 font-black text-slate-800 text-center border-t-2 border-indigo-200">
                                                    ...
                                                    <td class="px-3 py-2 font-black text-amber-700 bg-amber-500/10 border-l-2 border-r-2 border-amber-500/20 text-center text-xs shadow-inner"><span class="px-2.5 py-1 rounded-xl bg-amber-500 text-white font-black text-xs shadow-md shadow-amber-500/20">${grandFYBudget}</span></td>
                                                    ${grandCellsHTML}
                                                </tr>
                                            `;
                            ...
                            <div class="hidden md:block overflow-x-auto">
                                <table class="w-full text-left text-[11px] whitespace-nowrap border-collapse">
                                    <thead>
                                        <tr class="bg-slate-50/80 text-slate-500 uppercase tracking-widest text-[9px] border-b border-slate-200">
                                            <th class="px-6 py-2 font-black sticky left-0 z-10 bg-slate-50 border-r border-slate-200/80 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">Territory</th>
                                            ...
                                            ${app.adminShowYTD ? `<th class="px-3 py-2 text-center bg-gradient-to-b from-indigo-500/10 to-indigo-500/5 text-indigo-700 border-l border-r-2 border-t border-indigo-500/20 font-extrabold shadow-sm" colspan="4">YTD (${app.lastMonth.substring(0, 3)})</th>` : ''}
                                            ${app.adminShowLastMonth ? `<th class="px-3 py-2 text-center bg-gradient-to-b from-emerald-500/10 to-emerald-500/5 text-emerald-700 border-l border-r-2 border-t border-emerald-500/20 font-extrabold shadow-sm" colspan="3">Last Month (${app.lastMonth.substring(0, 3)})</th>` : ''}
                                        ...
                                        <tr class="text-slate-400 uppercase tracking-tighter text-[9px] border-b border-slate-200/80 text-center">
                                            <th class="px-6 py-1.5 sticky left-0 z-10 bg-white border-r border-slate-200 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                                                <div class="flex items-center justify-between gap-2">
                                                    <div class="flex items-center gap-1 cursor-pointer hover:text-indigo-600 transition-colors" onclick="app.setPulseSort('name')">
                                                        <span class="w-4 text-right text-slate-300">#</span> Territory ${app.getSortIcon('name')}
                                                    </div>
                                                    <button onclick="app.showPulseFilterModal()" class="p-1 rounded-md transition-colors tooltip ${app.pulseFilterTerritories && app.pulseFilterTerritories.length > 0 ? 'bg-indigo-100 text-indigo-700 shadow-inner scale-110' : 'hover:bg-slate-100 text-slate-400'}" title="Filter Territories">
                                                        <i data-lucide="filter" class="w-3.5 h-3.5"></i>
                                                    </button>
                                            ...
                                            <th class="px-3 py-1.5 bg-amber-500/[0.02] border-l-2 border-r-2 border-amber-500/20 cursor-pointer hover:bg-amber-100/50 transition-colors text-center font-bold text-amber-800" onclick="app.setPulseSort('sortVal_fy_budget')">
                                                <div class="flex items-center justify-center gap-1 font-black">
                                                    Total ${app.getSortIcon('sortVal_fy_budget')}
                                                </div>
                                            </th>
                                            ${app.adminShowYTD ? `
                                                <th class="px-2 py-1.5 bg-indigo-50/50 cursor-pointer hover:bg-indigo-100/50 transition-colors border-l border-slate-100/80" onclick="app.setPulseSort('sortVal_ytd_budget')"><div class="flex items-center justify-center gap-1">Budget ${app.getSortIcon('sortVal_ytd_budget')}</div></th>
                                                <th class="px-2 py-1.5 bg-indigo-50/50 cursor-pointer hover:bg-indigo-100/50 transition-colors" onclick="app.setPulseSort('sortVal_ytd_actual')"><div class="flex items-center justify-center gap-1">Actual ${app.getSortIcon('sortVal_ytd_actual')}</div></th>
                                                <th class="px-2 py-1.5 bg-indigo-50/50 cursor-pointer hover:bg-indigo-100/50 transition-colors" onclick="app.setPulseSort('sortVal_ytd_ach')"><div class="flex items-center justify-center gap-1">Ach% ${app.getSortIcon('sortVal_ytd_ach')}</div></th>
                                                <th class="px-2 py-1.5 bg-indigo-50/50 cursor-pointer hover:bg-indigo-100/50 transition-colors border-r-2 border-indigo-500/20" onclick="app.setPulseSort('sortVal_ytd_short')"><div class="flex items-center justify-center gap-1">Short ${app.getSortIcon('sortVal_ytd_short')}</div></th>
                                            ` : ''}
                                            ${app.adminShowLastMonth ? `
                                                <th class="px-2 py-1.5 bg-emerald-50/20 cursor-pointer hover:bg-emerald-100/30 transition-colors border-l border-slate-100/80" onclick="app.setPulseSort('sortVal_lm_budget')"><div class="flex items-center justify-center gap-1">Budget ${app.getSortIcon('sortVal_lm_budget')}</div></th>
                                                <th class="px-2 py-1.5 bg-emerald-50/20 font-bold text-slate-600 cursor-pointer hover:bg-emerald-100/30 transition-colors" onclick="app.setPulseSort('sortVal_lm_actual')"><div class="flex items-center justify-center gap-1">Actual ${app.getSortIcon('sortVal_lm_actual')}</div></th>
                                                <th class="px-2 py-1.5 bg-emerald-50/20 cursor-pointer hover:bg-emerald-100/30 transition-colors border-r-2 border-emerald-500/20" onclick="app.setPulseSort('sortVal_lm_ach')"><div class="flex items-center justify-center gap-1">Ach% ${app.getSortIcon('sortVal_lm_ach')}</div></th>
                                            ` : ''}
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        ${(() => {
                                            ...
                                            const rowsHTML = mappedTerritories.map((mt, idx) => {
                                                ...
                                                return `
                                                    <tr class="hover:bg-slate-50/50 transition-colors group text-center border-b border-slate-100">
                                                        <td class="px-6 py-1.5 text-left sticky left-0 z-10 bg-white border-r border-slate-200/85 shadow-[2px_0_5px_rgba(0,0,0,0.02)] font-medium">
                                                            <div class="flex items-center justify-between gap-2">
                                                                <div class="flex items-center gap-1.5">
                                                                    <span class="text-[9px] font-bold text-slate-400 w-4 text-right">${idx + 1}.</span>
                                                                    <div class="w-1.5 h-4.5 bg-${mt.h}-500 rounded-full"></div>
                                                                    <span class="font-black text-slate-700">${mt.name}</span>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td class="px-3 py-1.5 font-bold text-amber-800 bg-amber-500/[0.01] border-l-2 border-r-2 border-amber-500/10 text-center"><span class="px-2 py-0.5 rounded-lg text-amber-800 font-extrabold pulse-badge-amber text-[10px]">${mt.totalFYBudget}</span></td>
                                                        ${app.adminShowYTD ? `
                                                            <td class="px-2 py-1.5 text-slate-400 font-medium text-center border-l border-slate-100/80">${mt.perf.ytd.budget}</td>
                                                            <td class="px-2 py-1.5 font-black text-slate-700 text-center">${mt.perf.ytd.sales}</td>
                                                            <td class="px-2 py-1.5 text-center"><span class="px-1.5 py-0.5 rounded-lg bg-${mt.h}-50 text-${mt.h}-600 font-black">${mt.ytdAchVal}%</span></td>
                                                            <td class="px-2 py-1.5 font-bold text-rose-500 text-center border-r-2 border-indigo-500/20">${mt.ytdShortVal}</td>
                                                        ` : ''}
                                                        ${app.adminShowLastMonth ? `
                                                            <td class="px-2 py-1.5 text-slate-400 font-medium text-center border-l border-slate-100/80">${mt.perf.lastMonth.budget}</td>
                                                            <td class="px-2 py-1.5 font-black text-slate-700 text-center">${mt.perf.lastMonth.sales}</td>
                                                            <td class="px-2 py-1.5 font-black text-slate-600 text-center border-r-2 border-emerald-500/20">${mt.lmAchVal}%</td>
                                                        ` : ''}
                                            ...
                                            const totalRowHTML = `
                                                <tr class="bg-indigo-50/20 font-black text-slate-800 text-center border-t-2 border-indigo-200">
                                                    <td class="px-6 py-2 text-left sticky left-0 z-10 bg-indigo-50 border-r border-slate-200/85 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                                                        <div class="flex items-center gap-2">
                                                            <span class="w-4"></span>
                                                            <div class="w-1.5 h-4.5 bg-indigo-600 rounded-full shadow shadow-indigo-500/50"></div>
                                                            <span class="font-extrabold text-slate-800 text-[10px]">GRAND TOTAL</span>
                                                    ...
                                                    <td class="px-3 py-2 font-black text-amber-700 bg-amber-500/10 border-l-2 border-r-2 border-amber-500/20 text-center text-xs shadow-inner"><span class="px-2.5 py-1 rounded-xl bg-amber-500 text-white font-black text-xs shadow-md shadow-amber-500/20">${totalFYBudget}</span></td>
                                                    ${app.adminShowYTD ? `
                                                        <td class="px-2 py-2 text-slate-700 font-extrabold bg-indigo-50/10 text-center border-l border-slate-100/80">${totalYtdBudget}</td>
                                                        <td class="px-2 py-2 font-black text-slate-900 bg-indigo-50/10 text-center">${totalYtdSales}</td>
                                                        <td class="px-2 py-2 bg-indigo-50/10 text-center"><span class="px-1.5 py-0.5 rounded-lg bg-${totalH}-50 text-${totalH}-600 font-black">${totalYtdAchVal}%</span></td>
                                                        <td class="px-2 py-2 font-bold text-rose-600 bg-indigo-50/10 text-center border-r-2 border-indigo-500/20">${totalYtdShort}</td>
                                                    ` : ''}
                                                    ${app.adminShowLastMonth ? `
                                                        <td class="px-2 py-2 text-slate-700 font-extrabold text-center border-l border-slate-100/80">${totalLastMonthBudget}</td>
                                                        <td class="px-2 py-2 font-black text-slate-900 text-center">${totalLastMonthSales}</td>
                                                        <td class="px-2 py-2 font-black text-slate-800 text-center border-r-2 border-emerald-500/20">${ach(totalLastMonthSales, totalLastMonthBudget)}%</td>
                                                    ` : ''}
                            ...
                            <div class="md:hidden">
                                ...
                                <div class="divide-y divide-slate-100">
                                    ${mappedTerritories.map((mt, idx) => {
                                        ...
                                        const monthlyPerf = {};
                                        fiscalMonths.forEach(m => {
                                            const monthTgts = DB.targets.filter(tg => tg.territory_id === t.id && tg.brand === brandFilter && tg.sale_type === currentSaleType && tg.month === m);
                                            const mBudget = monthTgts.length > 0 ? monthTgts.reduce((sum, tg) => sum + Number(tg.target_qty || 0), 0) : Math.round(totalFYBudget / 12);
                                            const mSales = currFYSales.filter(s => s.territory_id === t.id && s.brand === brandFilter && s.sales_month === m).reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);
                                            const mSply = lastFYSales.filter(s => s.territory_id === t.id && s.brand === brandFilter && s.sales_month === m).reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);
                                            monthlyPerf[m] = {
                                        ...
                                        <div class="p-5 bg-white active:bg-slate-50 transition-colors">
                                            ...
                                            <div class="grid grid-cols-3 gap-2">
                                                ...
                                                <div class="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                                                    ...
                                                    <p class="text-sm font-black text-${h}-600">${ytdAchVal}%</p>
                                                </div>
                    ...
                    <div class="glass rounded-[2rem] border border-white shadow-xl overflow-hidden mb-8">
                        ...
                        <div class="hidden md:block overflow-x-auto">
                            <table class="w-full text-left text-[11px] whitespace-nowrap">
                                <thead>
                                    <tr class="bg-indigo-50/50 text-slate-500 uppercase tracking-widest text-[9px] border-b border-indigo-100">
                                        <th class="px-6 py-4 font-black sticky left-0 z-10 bg-indigo-50 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">Area Manager</th>
                                        <th class="px-6 py-4 font-black bg-indigo-50/50 border-r border-indigo-100">Area Name</th>
                                        ${app.adminShowYTD ? `<th class="px-3 py-4 text-center border-l border-indigo-100" colspan="3">YTD (${app.lastMonth.substring(0, 3)})</th>` : ''}
                                        ${app.adminShowLastMonth ? `<th class="px-3 py-4 text-center border-l border-indigo-100" colspan="2">Last Month (${app.lastMonth.substring(0, 3)})</th>` : ''}
                                        <th class="px-3 py-4 text-center border-l border-indigo-100" colspan="${4 + activeModels.length}">Current Month (${app.currentMonth.substring(0, 3)})</th>
                                    </tr>
                                    <tr class="text-slate-400 uppercase tracking-tighter text-[9px] border-b border-indigo-100 text-center">
                                        <th class="px-6 py-3 sticky left-0 z-10 bg-white shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]"></th>
                                        <th class="px-6 py-3 bg-white border-r border-indigo-100 text-left">
                                            <div class="flex items-center justify-between gap-2">
                                                <span>Area Name</span>
                                        ...
                                        ${app.adminShowLastMonth ? `
                                            ...
                                            <th class="px-2 py-3 font-bold text-slate-600">Actual</th>
                                        ` : ''}
                                        <th class="px-2 py-3 bg-indigo-50/30">Budget</th>
                                        <th class="px-2 py-3 bg-indigo-50/30 font-bold text-slate-600">Proj</th>
                                        ${activeModels.map(m => `<th class="px-2 py-3 bg-indigo-50/30">${m}</th>`).join('')}
                                        <th class="px-2 py-3 bg-indigo-600 font-black text-white">Total</th>
                                        <th class="px-2 py-3 bg-indigo-600 font-black text-white">Ach%</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-50 bg-white">
                                    ${areaStats.map(area => {
                            const modelCells = activeModels.map(mName => {
                                ...
                                return `<td class="px-2 py-4 bg-slate-50/30 ${qty > 0 ? 'font-bold text-slate-700' : 'text-slate-300'}">${qty > 0 ? qty : '-'}</td>`;
                            }).join('');
                            const totalSalesDisplay = area.currSales > 0 ? `<div class="font-bold text-sm">${area.currSales}</div>` : '<span class="text-slate-400">0</span>';

                            return `
                                            <tr class="hover:bg-indigo-50/20 transition-colors text-center">
                                                <td class="px-6 py-4 text-left sticky left-0 z-10 bg-white border-r border-slate-50 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                                                    <div class="flex items-center gap-2">
                                                        <div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 text-[10px] font-black shrink-0">${area.name.charAt(0)}</div>
                                                        <span class="font-black text-slate-700">${area.name}</span>
                                                    </div>
                                                </td>
                                                <td class="px-6 py-4 text-left font-bold text-indigo-600 border-r border-indigo-50 bg-slate-50/30 whitespace-nowrap">
                                                    ${area.areaName}
                                                </td>
                                                ${app.adminShowYTD ? `
                                                    <td class="px-2 py-4 text-slate-400">${area.ytd.budget}</td>
                                                    <td class="px-2 py-4 font-black text-slate-700">${area.ytd.sales}</td>
                                                    <td class="px-2 py-4 font-black text-indigo-600">${ach(area.ytd.sales, area.ytd.budget)}%</td>
                                                ` : ''}
                                                ${app.adminShowLastMonth ? `
                                                    <td class="px-2 py-4 text-slate-400">${area.lastMonth.budget}</td>
                                                    <td class="px-2 py-4 font-black text-slate-700">${area.lastMonth.sales}</td>
                                                ` : ''}
                                                <td class="px-2 py-4 bg-slate-50/50 text-slate-400">${area.currBudget}</td>
                                                <td class="px-2 py-4 bg-slate-50/50 font-black text-slate-700">${area.currProj}</td>
                                                ${modelCells}
                                                <td class="px-2 py-4 bg-indigo-600 font-black text-white">${totalSalesDisplay}</td>
                                                <td class="px-2 py-4 bg-indigo-600 font-black text-white">${ach(area.currSales, area.currBudget)}%</td>
                                            </tr>
                                        `;
                        }).join('')}
                                </tbody>
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="flex items-center gap-1.5 mt-0.5">
                                                        ...
                                                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Active Leader</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="text-right">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            ...
                                            <div class="bg-emerald-50/50 p-3 rounded-2xl border border-emerald-100">
                                                <div class="flex justify-between items-center mb-1">
                                                    ...
                                                    <span class="text-[10px] font-black text-emerald-700">${ytdAchVal}%</span>
                                                </div>
                                                <div class="w-full bg-white h-1.5 rounded-full overflow-hidden border border-emerald-100">
                                                    <div class="h-full bg-emerald-500 rounded-full" style="width: ${Math.min(100, ytdAchVal)}%"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            renderAdminAIInsights: () => {
                ...
                const currentSales = DB.sales.filter(s => s.fy === currentFY && s.sales_month === currentMonth);
                ...
                DB.targets.filter(t => t.fy === currentFY).forEach(t => {
                    const monthlyTarget = t.month === currentMonth ? t.target_qty : (t.month ? 0 : Math.round(t.target_qty / 12));
                    totalTarget += monthlyTarget;
                    if (brandStats[t.brand]) brandStats[t.brand].target += monthlyTarget;
                });

                DB.projections.filter(p => p.fy === currentFY && p.month === currentMonth).forEach(p => {
                    totalProjection += p.projection_qty;
                    if (brandStats[p.brand]) brandStats[p.brand].projection += p.projection_qty;
                });

                // LY Stats for Growth Analysis
                DB.sales.filter(s => s.fy === '2024-25' && s.sales_month === currentMonth).forEach(s => {
                    if (brandStats[s.brand]) brandStats[s.brand].lyActual += Number(s.unit_qty || 0);
                });

                // AI Predictions
                const dailyRate = totalActual / currentDay;
                const predictedClose = Math.round(dailyRate * daysInMonth);
                ...
                const terrPerformance = DB.territories.map(t => {
                    ...
                    const tTarget = DB.targets.filter(tg => tg.territory_id === t.id && tg.fy === currentFY).reduce((sum, tg) => {
                        const mTarget = tg.month === currentMonth ? tg.target_qty : (tg.month ? 0 : Math.round(tg.target_qty / 12));
                        return sum + mTarget;
                    }, 0);
                ...
                currentSales.forEach(s => { modelTotalSales[s.model] += Number(s.unit_qty || 0); });

                const modelInsights = DB.models.map(m => ({
                    ...m,
                    sales: modelTotalSales[m.name],
                    performance: modelTotalSales[m.name] > 10 ? 'High' : (modelTotalSales[m.name] > 5 ? 'Medium' : 'Low')
                ...
                const brandDominance = DB.territories.map(t => {
                    const tSales = currentSales.filter(s => s.territory_id === t.id);
                ...
                const html = `
                    <div class="max-w-7xl mx-auto fade-in pb-12">
                        <!-- AI Header Section -->
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6 px-2 md:px-0">
                            <div class="w-full md:w-auto">
                                <h1 class="text-3xl md:text-5xl font-black text-slate-900 tracking-tight flex items-center gap-3 md:gap-4">
                                    <div class="p-2.5 md:p-3.5 bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl md:rounded-[2rem] shadow-xl shadow-purple-200 animate-pulse">
                                        <i data-lucide="brain-circuit" class="w-6 h-6 md:w-10 md:h-10 text-white"></i>
                                    </div>
                                    <span class="leading-none">AI Strategic<br class="md:hidden"><span class="text-purple-600"> Insights</span></span>
                                </h1>
                                <p class="text-slate-500 font-bold text-xs mt-3 flex items-center gap-2">
                                    <span class="flex h-2 w-2 relative">
                                        <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                                        <span class="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
                                    </span>
                                    Autonomous Engine FY ${currentFY}
                                </p>
                            </div>
                            <div class="flex items-center gap-2 bg-white/80 backdrop-blur-md p-2 rounded-[1.5rem] shadow-xl border border-white w-full md:w-auto overflow-hidden">
                                <div class="px-3 py-1 flex-1 md:flex-none text-center">
                                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Confidence</p>
                                    <p class="text-lg font-black text-purple-600">${predictionConfidence}%</p>
                                </div>
                                <div class="w-px h-8 bg-slate-200"></div>
                                <div class="px-3 py-1 flex-1 md:flex-none text-center">
                                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Analysis</p>
                                    <p class="text-lg font-black text-slate-800">${currentMonth}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <!-- Prediction Card -->
                            <div class="glass p-6 rounded-[2.5rem] border border-white shadow-2xl relative overflow-hidden group">
                                <div class="absolute -right-10 -top-10 w-40 h-40 bg-purple-500/10 rounded-full blur-3xl transition-transform group-hover:scale-150 duration-700"></div>
                                ...
                                <div class="flex items-baseline gap-2">
                                    <span class="text-5xl font-black text-slate-900 tracking-tighter">${predictedClose}</span>
                                    <span class="text-sm font-bold text-slate-400 uppercase">Units</span>
                                </div>
                                <div class="mt-6 flex items-center justify-between">
                                    <div class="flex flex-col">
                                        <span class="text-[10px] font-bold text-slate-400 uppercase">Pace vs Budget</span>
                                        <span class="text-sm font-black ${isPacingWell ? 'text-green-600' : 'text-amber-600'}">
                                            ${isPacingWell ? 'Ahead of Schedule' : 'Slightly Behind'}
                                        </span>
                                    </div>
                                    <div class="h-12 w-12 rounded-full border-4 ${isPacingWell ? 'border-green-100' : 'border-amber-100'} flex items-center justify-center relative">
                                        <svg class="w-full h-full transform -rotate-90">
                                            <circle cx="24" cy="24" r="20" fill="transparent" stroke="currentColor" stroke-width="4" class="${isPacingWell ? 'text-green-500' : 'text-amber-500'}" stroke-dasharray="125.6" stroke-dashoffset="${125.6 * (1 - Math.min(predictedClose / totalTarget, 1))}"/>
                                        </svg>
                                        <i data-lucide="${isPacingWell ? 'check' : 'alert-triangle'}" class="absolute w-4 h-4 ${isPacingWell ? 'text-green-500' : 'text-amber-500'}"></i>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Brand Momentum Card -->
                            <div class="glass p-6 rounded-[2.5rem] border border-white shadow-2xl relative overflow-hidden group">
                                <div class="absolute -right-10 -top-10 w-40 h-40 bg-blue-500/10 rounded-full blur-3xl transition-transform group-hover:scale-150 duration-700"></div>
                                <h3 class="text-slate-500 text-xs font-black uppercase tracking-widest mb-4 flex items-center gap-2">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <div class="flex justify-between text-[10px] font-black mb-1 uppercase tracking-tighter">
                            ...
                            <!-- Quick Insight Card -->
                            <div class="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-2xl relative overflow-hidden group text-white">
                                <div class="absolute -right-4 -bottom-4 opacity-10 group-hover:scale-110 transition-transform duration-500">
                                    <i data-lucide="cpu" class="w-32 h-32 text-purple-400"></i>
                                </div>
                                <h3 class="text-slate-400 text-xs font-black uppercase tracking-widest mb-4 flex items-center gap-2">
                                    <i data-lucide="sparkles" class="w-4 h-4 text-purple-400"></i>
                                    Deep Data Synthesis
                                </h3>
                                <p class="text-sm font-medium leading-relaxed text-slate-300 mb-4">
                                    National growth is <strong>${Math.round(((totalActual - (brandStats.Foton.lyActual + brandStats.Mahindra.lyActual)) / (brandStats.Foton.lyActual + brandStats.Mahindra.lyActual || 1)) * 100)}%</strong> vs SPLY. ${predictedClose > totalTarget ? 'Target surplus projected.' : 'Daily volume gap identified: +' + Math.ceil((totalTarget - predictedClose) / (daysInMonth - currentDay)) + ' units/day needed.'}
                                </p>
                                <div class="flex items-center gap-4">
                                    <div class="flex -space-x-2">
                                        <div class="w-6 h-6 rounded-full bg-purple-500 border border-slate-900 flex items-center justify-center text-[8px] font-bold">AI</div>
                                        <div class="w-6 h-6 rounded-full bg-indigo-500 border border-slate-900 flex items-center justify-center text-[8px] font-bold">BI</div>
                                    </div>
                                    <span class="text-[9px] font-black uppercase tracking-widest text-slate-500">Multi-Model Analysis</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                            ...
                            <div class="glass p-8 rounded-[2.5rem] border border-white shadow-xl flex flex-col">
                                <div class="flex items-center justify-between mb-6">
                                    <h3 class="text-xl font-black text-slate-800 tracking-tight">Upazila Champion Models</h3>
                                    <div class="p-2 bg-indigo-50 text-indigo-600 rounded-xl"><i data-lucide="map-pin" class="w-5 h-5"></i></div>
                                </div>
                                <div class="space-y-4 flex-1">
                                    ${upazilaAffinities.map(a => `
                                        <div class="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100 group hover:border-indigo-300 transition-all">
                                            <div>
                                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">${a.upazila}</p>
                                                <p class="font-black text-slate-800 text-sm">${a.model}</p>
                                            </div>
                                            <div class="text-right">
                                                <span class="px-2 py-1 bg-white rounded-lg border border-slate-200 text-xs font-black text-indigo-600 shadow-sm">${a.qty} Units</span>
                                            </div>
                                        </div>
                                    `).join('')}
                                </div>
                                <p class="text-[9px] text-slate-400 font-bold mt-6 uppercase tracking-widest text-center italic">AI identified high-affinity clusters based on regional use-cases</p>
                            </div>

                            <!-- Column 2: Brand Dominance Battleground -->
                            <div class="glass p-8 rounded-[2.5rem] border border-white shadow-xl flex flex-col">
                                <div class="flex items-center justify-between mb-6">
                                    <h3 class="text-xl font-black text-slate-800 tracking-tight">Regional Brand Dominance</h3>
                                    <div class="p-2 bg-rose-50 text-rose-600 rounded-xl"><i data-lucide="swords" class="w-5 h-5"></i></div>
                                </div>
                                <div class="space-y-6 flex-1">
                                    ${brandDominance.map(t => `
                                        <div>
                                            <div class="flex justify-between items-end mb-2">
                                                <span class="text-xs font-black text-slate-700">${t.name}</span>
                                                <span class="text-[10px] font-black text-${t.leaderColor}-600 uppercase tracking-widest">Leading: ${t.leader}</span>
                                            </div>
                                            <div class="w-full bg-slate-100 h-3 rounded-full overflow-hidden flex p-0.5 border border-slate-200">
                                ...
                                <div class="mt-8 flex items-center justify-center gap-6">
                                    <div class="flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-indigo-500"></span><span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Foton</span></div>
                                    <div class="flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-rose-500"></span><span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Mahindra</span></div>
                                </div>
                            </div>

                            <!-- Column 3: Risk & Underperforming Assets -->
                            <div class="glass p-8 rounded-[2.5rem] border border-white shadow-xl flex flex-col bg-slate-50/50">
                                <div class="flex items-center justify-between mb-6">
                                    <h3 class="text-xl font-black text-slate-800 tracking-tight text-rose-700">Model Performance Risks</h3>
                                    <div class="p-2 bg-rose-100 text-rose-600 rounded-xl shadow-inner"><i data-lucide="alert-octagon" class="w-5 h-5"></i></div>
                                </div>
                                <div class="space-y-4 flex-1">
                                    ${underperformers.map(m => `
                                        <div class="p-4 rounded-2xl bg-white border border-rose-100 shadow-sm relative overflow-hidden group">
                                            <div class="absolute right-0 top-0 w-12 h-full bg-rose-50 -skew-x-12 translate-x-4 opacity-0 group-hover:opacity-100 transition-all"></div>
                                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">${m.brand}</p>
                                            <div class="flex justify-between items-center relative z-10">
                                                <span class="font-black text-slate-800">${m.name}</span>
                                                <div class="text-right">
                                                    <span class="text-rose-600 font-black text-sm">${m.sales} Sales</span>
                                                    <p class="text-[8px] font-black text-rose-400 uppercase tracking-tighter">Underperforming</p>
                                                </div>
                                            </div>
                                        </div>
                                    `).join('')}
                                    ${underperformers.length === 0 ? '<p class="text-center text-slate-400 font-bold py-10">All models performing within threshold.</p>' : ''}
                                </div>
                                <div class="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                                    <p class="text-[10px] font-black text-amber-800 uppercase tracking-widest mb-1 flex items-center gap-2"><i data-lucide="info" class="w-3 h-3"></i> AI Recommendation</p>
                                    <p class="text-[11px] leading-relaxed text-amber-700 font-medium italic">"Low velocity in specific models suggests stock-heavy territories or price-sensitivity in rural clusters. Consider model-specific exchange campaigns."</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            <div class="flex items-center gap-3 mb-6">
                                <h2 class="text-2xl font-black text-slate-800 tracking-tight">Quantum Market Intelligence</h2>
                                <span class="bg-purple-100 text-purple-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border border-purple-200">Experimental</span>
                            </div>
                            
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                <!-- Predictive Seasonal Shift -->
                                <div class="bg-gradient-to-br from-indigo-900 to-slate-900 p-6 rounded-[2rem] text-white shadow-xl relative overflow-hidden group">
                                    ...
                                    <h4 class="text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-4 flex items-center gap-2">
                                        <i data-lucide="calendar-days" class="w-4 h-4"></i>
                                        Next Month Forecast (May)
                                    </h4>
                                    <div class="space-y-3">
                                        <div class="flex justify-between items-center">
                                            <span class="text-xs font-medium text-slate-300">Projected Demand</span>
                                            <span class="text-lg font-black text-indigo-400">+12.4%</span>
                                        </div>
                                        <div class="p-3 bg-white/5 rounded-xl border border-white/10">
                                            <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Trending Model</p>
                                            <p class="text-sm font-black text-white">Mahindra Bolero Pick-up</p>
                                        </div>
                                    </div>
                                    <div class="mt-6 pt-4 border-t border-white/10">
                                        <p class="text-[9px] text-slate-400 leading-relaxed italic">"Historical data suggests seasonal uptick in agricultural logistics demand starting May."</p>
                                    </div>
                                </div>

                                <!-- Territory Growth Clustering -->
                                <div class="glass p-6 rounded-[2rem] border border-white shadow-xl relative overflow-hidden flex flex-col">
                                    ...
                                    <div class="space-y-3 flex-1">
                                        <div class="flex items-center justify-between p-2 rounded-xl bg-emerald-50 border border-emerald-100">
                                            <span class="text-[10px] font-black text-emerald-700 uppercase">Hyper-Growth</span>
                                            <span class="text-xs font-black text-emerald-800">2 Regions</span>
                                        </div>
                                        <div class="flex items-center justify-between p-2 rounded-xl bg-blue-50 border border-blue-100">
                                            <span class="text-[10px] font-black text-blue-700 uppercase">Steady State</span>
                                            <span class="text-xs font-black text-blue-800">5 Regions</span>
                                        </div>
                                        <div class="flex items-center justify-between p-2 rounded-xl bg-rose-50 border border-rose-100">
                                            <span class="text-[10px] font-black text-rose-700 uppercase">Reactivation Needed</span>
                                            <span class="text-xs font-black text-rose-800">1 Region</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="glass p-6 rounded-[2rem] border border-white shadow-xl relative overflow-hidden flex flex-col">
                                    ...
                                    <div class="flex-1 flex flex-col items-center justify-center py-2">
                                        <div class="relative w-20 h-20 flex items-center justify-center">
                                            <svg class="w-full h-full transform -rotate-90">
                                                <circle cx="40" cy="40" r="36" fill="transparent" stroke="#f1f5f9" stroke-width="8"/>
                                                <circle cx="40" cy="40" r="36" fill="transparent" stroke="#8b5cf6" stroke-width="8" stroke-dasharray="226" stroke-dashoffset="${226 * (1 - 0.92)}"/>
                                            </svg>
                                            <span class="absolute text-lg font-black text-slate-800">92%</span>
                                        </div>
                                        <p class="text-[10px] font-black text-purple-600 uppercase mt-3 tracking-tighter">Very High Confidence</p>
                                    </div>
                                </div>
                                ...
                                <div class="glass p-6 rounded-[2rem] border border-white shadow-xl relative overflow-hidden flex flex-col">
                                    ...
                                    <div class="space-y-4 flex-1 justify-center flex flex-col">
                                        <div class="flex justify-between text-xs font-bold">
                                            <span class="text-slate-500">Critical Stock-out In:</span>
                                            <span class="text-rose-600 font-black">18 Days</span>
                                        </div>
                                        <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                            <div class="bg-rose-500 h-full rounded-full" style="width: 75%"></div>
                                        </div>
                                        <p class="text-[8px] text-slate-400 font-bold uppercase tracking-widest text-center">Based on current Daily Sales Velocity (DSV)</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="lg:col-span-2 space-y-6">
                                <div class="glass p-8 rounded-[2.5rem] border border-white shadow-xl relative overflow-hidden">
                                    <div class="flex items-center justify-between mb-8">
                                        <h2 class="text-2xl font-black text-slate-800 tracking-tight">Primary AI Interventions</h2>
                                        <div class="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-indigo-100">Strategic Priority</div>
                                    </div>
                                    
                                    <div class="space-y-6">
                                        <!-- Recommendation 1 -->
                                        ${atRiskTerritories.length > 0 ? `
                                        <div class="flex gap-6 items-start group">
                                            <div class="shrink-0 w-12 h-12 bg-emerald-100 rounded-2xl flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-all duration-300 shadow-sm">
                                                <i data-lucide="crosshairs" class="w-6 h-6"></i>
                                            </div>
                                            <div>
                                                <h4 class="font-black text-slate-800 text-lg mb-1">Focus on ${atRiskTerritories[0].name} Territory</h4>
                                                <p class="text-sm text-slate-500 font-medium leading-relaxed">
                                                    AI detects a critical conversion gap in <strong>${atRiskTerritories[0].name}</strong>. Current achievement is only ${atRiskTerritories[0].ach}%. Recommend immediate reallocation of 20% marketing budget to this zone for localized activations.
                                                </p>
                                            </div>
                                        </div>
                                        ` : `
                                        <div class="flex gap-6 items-start group">
                                            ...
                                            <div>
                                                <h4 class="font-black text-slate-800 text-lg mb-1">All Territories Healthy</h4>
                                                <p class="text-sm text-slate-500 font-medium leading-relaxed">
                                                    AI confirms no critical conversion gaps across regions. Maintain current operational velocity to secure the FY Target.
                                                </p>
                                            </div>
                                        </div>
                                        `}
                                        ...
                                        <div class="flex gap-6 items-start group">
                                            <div class="shrink-0 w-12 h-12 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 shadow-sm">
                                                <i data-lucide="trending-up" class="w-6 h-6"></i>
                                            </div>
                                            <div>
                                                <h4 class="font-black text-slate-800 text-lg mb-1">Leverage ${topTerritories[0].name} Success</h4>
                                                <p class="text-sm text-slate-500 font-medium leading-relaxed">
                                                    <strong>${topTerritories[0].name}</strong> has surpassed its monthly quota pace. Our analysis indicates their localized strategy is working. Push this directive to other territories via the Notice Board.
                                                </p>
                                            </div>
                                        </div>
                                        ` : ''}
                                        ...
                                        <div class="flex gap-6 items-start group">
                                            <div class="shrink-0 w-12 h-12 bg-amber-100 rounded-2xl flex items-center justify-center text-amber-600 group-hover:bg-amber-600 group-hover:text-white transition-all duration-300 shadow-sm">
                                                <i data-lucide="zap" class="w-6 h-6"></i>
                                            </div>
                                            <div>
                                                <h4 class="font-black text-slate-800 text-lg mb-1">Stock Optimization Alert</h4>
                                                <p class="text-sm text-slate-500 font-medium leading-relaxed">
                                                    Demand for <strong>${upazilaAffinities[0].model}</strong> in <strong>${upazilaAffinities[0].upazila}</strong> is significantly higher than historical average. AI recommends prepositioning inventory in regional hubs to prevent "Stock-Out" scenarios.
                                                </p>
                                            </div>
                                        </div>
                                        ` : ''}
                                ...
                                <div class="glass p-8 rounded-[2.5rem] border border-white shadow-xl">
                                    <h2 class="text-2xl font-black text-slate-800 tracking-tight mb-6">Territory Health Matrix</h2>
                                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                        ${terrPerformance.map(t => `
                                            <div class="p-4 rounded-3xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:shadow-lg transition-all duration-300 group">
                                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 truncate">${t.name}</p>
                                                <div class="flex justify-between items-end">
                                                    <span class="text-lg font-black text-slate-800">${t.ach}%</span>
                                                    <div class="w-8 h-8 rounded-full flex items-center justify-center ${t.ach > 70 ? 'bg-green-100 text-green-600' : (t.ach > 40 ? 'bg-amber-100 text-amber-600' : 'bg-red-100 text-red-600')}">
                                                        <i data-lucide="${t.ach > 70 ? 'smile' : (t.ach > 40 ? 'neutral' : 'frown')}" class="w-4 h-4"></i>
                                                    </div>
                                                </div>
                            ...
                            <div class="space-y-8">
                                ...
                                <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl">
                                    <h3 class="text-lg font-black text-slate-800 flex items-center gap-2 mb-6">
                                    ...
                                    <div class="space-y-4">
                                        ${topTerritories.map((t, idx) => `
                                            <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                <div class="flex items-center gap-3">
                                                    <div class="w-8 h-8 rounded-xl bg-amber-500 text-white flex items-center justify-center font-black text-sm">${idx + 1}</div>
                                                    <div>
                                                        <p class="font-black text-slate-800 text-sm">${t.name}</p>
                                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">${t.actual} Units Sold</p>
                                                    </div>
                                                </div>
                                                <div class="text-right">
                                                    <p class="text-sm font-black text-green-600">${t.ach}%</p>
                                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Achieved</p>
                                                </div>
                                            </div>
                                        `).join('')}
                                    </div>
                                ...
                                <!-- Risk Monitor -->
                                <div class="bg-rose-50 p-8 rounded-[2.5rem] border border-rose-100 shadow-xl">
                                    <h3 class="text-lg font-black text-rose-800 flex items-center gap-2 mb-6">
                                        <i data-lucide="activity" class="w-5 h-5 text-rose-600"></i>
                                        Risk Monitor
                                    </h3>
                                    <div class="space-y-4">
                                        ${atRiskTerritories.length > 0 ? atRiskTerritories.map(t => `
                                            <div class="flex items-center justify-between p-4 bg-white/50 rounded-2xl border border-rose-100">
                                                <div>
                                                    <p class="font-black text-rose-900 text-sm">${t.name}</p>
                                                    <p class="text-[10px] font-bold text-rose-400 uppercase tracking-widest">Gap: ${t.target - t.actual} Units</p>
                                                </div>
                                                <div class="text-right">
                                                    <p class="text-sm font-black text-rose-600">${t.ach}%</p>
                                                    <p class="text-[10px] font-bold text-rose-400 uppercase tracking-widest">Critical</p>
                                                </div>
                                            </div>
                                        `).join('') : '<p class="text-center text-xs font-bold text-slate-400 py-4">No territories under 40% achievement.</p>'}
                                    </div>
                                    <button onclick="app.renderAdminDashboard()" class="w-full mt-6 bg-rose-600 hover:bg-rose-700 text-white font-black py-3 rounded-2xl shadow-lg shadow-rose-200 transition-all active:scale-95 text-xs uppercase tracking-widest">
                                        Review All Regions
                                    </button>
                                </div>
                            </div>
            ...
            renderAdminSalesMap: (keepDropdownOpen = false) => {
                ...
                // Robust Name Normalization for Map Polygons (Handles Old vs New English spellings)
                ...
                Object.entries(dataAgg).forEach(([k, v]) => {
                    normalizedAgg[getNormalizedKey(k)] = v;
                });

                const activeModels = brandTab === 'All' ? DB.models : DB.models.filter(m => m.brand === brandTab);
                const allDistricts = [...new Set(DB.territories.map(t => t.district))].sort();
                ...
                const html = `
                    <div class="max-w-7xl mx-auto fade-in pb-10 h-full flex flex-col">
                        ...
                        <!-- Map & Insights Layout -->
                        <div class="flex flex-col lg:flex-row gap-6 flex-1 min-h-[600px] relative z-10">
                            
                            <!-- Leaflet Map Container -->
                            <div class="flex-1 glass rounded-2xl relative overflow-hidden bg-slate-50 border border-slate-200 shadow-inner flex flex-col min-h-[500px] lg:min-h-0">
                            ...
                            <div class="w-full lg:w-80 flex flex-col gap-4 shrink-0">
                                <!-- High Level KPI -->
                                <div class="bg-gradient-to-br from-emerald-600 to-teal-700 p-5 rounded-2xl shadow-lg border border-emerald-500 relative overflow-hidden text-white">
                                    <i data-lucide="bar-chart-2" class="absolute -right-4 -bottom-4 w-24 h-24 text-emerald-400 opacity-20"></i>
                                    <p class="text-[10px] font-bold uppercase tracking-widest text-emerald-100 mb-1">Total Plotted Units</p>
                                    <h3 class="text-4xl font-black mb-1">${totalPlotted}</h3>
                                    <p class="text-xs text-emerald-100 font-medium">Visible in ${districtTab === 'All' ? 'Bangladesh' : districtTab}</p>
                                </div>

                                <!-- Leaderboard -->
                                <div class="glass flex-1 rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden max-h-[400px] lg:max-h-none">
                                    <div class="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between gap-2">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <h3 class="font-bold text-slate-800 text-sm">Top Performing Areas</h3>
                                        </div>
                                        <span class="text-[10px] uppercase font-bold text-slate-400 tracking-wider">${viewMode}</span>
                                    </div>
                                    <div class="flex-1 overflow-y-auto p-2">
                                        <div class="space-y-1">
                                            ${rankedAreas.length > 0 ? rankedAreas.map((u, idx) => `
                                                <div class="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 transition-colors group">
                                                    <div class="flex items-center gap-3">
                                                        <div class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${idx === 0 ? 'bg-amber-100 text-amber-600 shadow-sm border border-amber-200' : (idx === 1 ? 'bg-slate-200 text-slate-600 border border-slate-300' : (idx === 2 ? 'bg-orange-100 text-orange-700 border border-orange-200' : 'bg-slate-100 text-slate-400 border border-slate-200'))}">${idx + 1}</div>
                                                        <span class="font-bold text-sm text-slate-700 group-hover:text-emerald-600 transition-colors">${u.name}</span>
                                                    </div>
                                                    <span class="font-black text-slate-800">${u.qty}</span>
                                                </div>
                                            `).join('') : '<p class="text-center text-slate-400 text-sm py-8 font-medium">No areas to display</p>'}
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                // Coordinates for fallback if GeoJSON fails
                const mapCoords = {
                    // Upazilas
                ...
                setTimeout(async () => {
                    ...
                    L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager_labels_under/{z}/{x}/{y}{r}.png', {
                        subdomains: 'abcd', maxZoom: 12, minZoom: 6
                    }).addTo(app.salesMap);
                    ...
                    try {
                        const geoUrl = viewMode === 'district'
                            ? 'https://cdn.jsdelivr.net/gh/fahimreza-pro/bangladesh-geojson@master/bd-districts.json'
                            : 'https://cdn.jsdelivr.net/gh/fahimreza-pro/bangladesh-geojson@master/bd-upazilas.json';
                        ...
                        if (districtTab !== 'All') {
                            const normSelectedDist = getNormalizedKey(districtTab);
                            const filteredFeatures = geoData.features.filter(f => {
                                const fDist = viewMode === 'district'
                                    ? (f.properties.name || f.properties.NAME_2 || f.properties.ADM2_EN || '')
                                    : (f.properties.NAME_2 || f.properties.district || f.properties.ADM2_EN || '');
                                return getNormalizedKey(fDist) === normSelectedDist;
                            });
                        ...
                        const style = (feature) => {
                            const propName = feature.properties.name || feature.properties.NAME_2 || feature.properties.NAME_3 || '';
                            const normProp = getNormalizedKey(propName);
                        ...
                        const onEachFeature = (feature, layer) => {
                            ...
                            const propName = feature.properties.name || feature.properties.NAME_2 || feature.properties.NAME_3 || 'Unknown Area';
                            const normProp = getNormalizedKey(propName);
                            const sales = normalizedAgg[normProp] || 0;
                        ...
                        // Auto-focus map. If specific district is selected, it will zoom exactly to it due to filtered geoData.
                        if (districtTab !== 'All' || totalPlotted > 0) {
                            app.salesMap.fitBounds(app.geoLayer.getBounds(), { padding: [20, 20] });
                        }
                ...
                const html = `
                    <div class="max-w-6xl mx-auto pb-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <!-- 5. Last Year Sales Data -->
                            <div class="glass p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col group transition-all hover:shadow-md">
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 class="font-bold text-slate-800 flex items-center gap-2"><i data-lucide="history" class="w-5 h-5 text-rose-600 group-hover:scale-110 transition-transform"></i> Last Year Sales</h3>
                                        <p class="text-[10px] text-slate-500 mt-1">Historical FY data for YOY analysis</p>
                                ...
                                <div class="border-2 border-dashed border-slate-300 rounded-lg p-5 text-center hover:bg-slate-50 transition-colors cursor-pointer flex-1 flex flex-col justify-center" onclick="document.getElementById('file-last-year').click()">
                                    ...
                                    <p class="text-xs font-semibold text-slate-700">Upload LY Sales CSV</p>
                                    <input type="file" id="file-last-year" class="hidden" accept=".csv" onchange="app.simulateUpload(this, 'Last Year Sales Data', 'last_year_sales')">
                                </div>
                            </div>
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    ...
                                    <div class="space-y-3 p-4 rounded-2xl hover:bg-white/50 transition-colors">
                                        ...
                                        <div class="pl-10">
                                            <p class="text-[11px] font-bold text-rose-600 uppercase mb-1">Upload: Last Year Sales</p>
            ...
            viewUploadedData: (type) => {
                ...
                if (type === 'targets') {
                    ...
                    icon = 'target';
                    themeColor = 'indigo';
                    headers = ['FY', 'Month', 'Territory_Name', 'Upazila', 'Brand', 'Sale_Type', 'Target_Qty'];
                    data = DB.targets;
                    keyField = 'target_qty';
                    ...
                    title = 'Last Year Sales (FY 2024-25)';
                    icon = 'history';
                    themeColor = 'rose';
                    headers = ['Customer_ID', 'District', 'Territory_Name', 'Upazila', 'Brand', 'Model', 'Unit_Qty', 'FY', 'Sales_Year', 'Sales_Month', 'Sale_Type'];
                    data = DB.sales.filter(s => s.fy === '2024-25');
                    keyField = 'unit_qty';
                ...
                const html = `
                                ...
                                <table class="w-full text-left text-sm whitespace-nowrap">
                                    <thead class="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase text-[10px] font-bold tracking-widest">
                                        <tr>
                                            ...
                                            ${headers.map(h => `<th class="px-6 py-5">${h}</th>`).join('')}
                    ...
                    let rowCells = [];
                    if (type === 'targets') rowCells = [item.fy, item.month, DB.territories.find(t => t.id === item.territory_id)?.name || item.territory_id, item.upazila, item.brand, item.sale_type, `<span class="font-bold text-slate-800">${item.target_qty}</span>`];
                    ...
                    return `
                                                <tr id="row-${item.id}" class="hover:bg-slate-50/80 transition-colors group">
                                                    ...
                                                    ${rowCells.map((cell, cIdx) => `<td class="px-6 py-4 ${cIdx === 0 ? 'font-bold text-slate-700' : 'text-slate-500'}">${cell}</td>`).join('')}
                                                    <td class="px-6 py-4 text-right">
                                                        <div class="flex justify-end gap-2">
                                                            <button onclick="app.editRowData('${type}', '${item.id}')" class="p-2 text-slate-400 hover:text-aci-blue hover:bg-aci-blue/5 rounded-lg transition-all" title="Edit Row">
                                                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                                                            </button>
                                                            <button onclick="app.deleteSingleRow('${type}', '${item.id}')" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="Delete Row">
            ...
            deleteAllRows: (type) => {
                ...
                if (type === 'targets') DB.targets = [];
                else if (type === 'projections') DB.projections = [];
                else if (type === 'sales') DB.sales = DB.sales.filter(s => s.fy !== '2025-26');
                else if (type === 'last_year_sales') DB.sales = DB.sales.filter(s => s.fy !== '2024-25');
                else if (type === 'emi') DB.emi = [];
                else if (type === 'recovery_od') DB.recovery_od = [];
            ...
            editRowData: (type, id) => {
                ...
                const editFields = [];
                if (type === 'targets') editFields = ['fy', 'month', 'territory_id', 'upazila', 'brand', 'sale_type', 'target_qty'];
            ...
            downloadTemplate: (type) => {
                ...
                if (type === 'target') {
                    headers = "FY,Month,Territory_Name,Upazila,Brand,Sale_Type,Target_Qty\n";
                    headers += "2025-26,April,Dhaka North,Mirpur,Foton,New Sale,4\n";
                    filename = "ACI_Monthly_Target_Template.csv";
                } else if (type === 'projection') {
                    headers = "FY,Month,Territory_Name,Brand,Sale_Type,Projection_Qty\n";
                    headers += "2025-26,April,Dhaka North,Foton,New Sale,15\n";
                    headers += "2025-26,April,Dhaka North,Mahindra,Resale,10\n";
                    filename = "ACI_Current_Month_Projection_Template.csv";
                } else if (type === 'sales') {
                    headers = "Customer_ID,District,Territory_Name,Upazila,Brand,Model,Unit_Qty,FY,Sales_Year,Sales_Month,Sale_Type\n";
                    headers += "C-1001,Dhaka,Dhaka North,Mirpur,Foton,TM3,1,2025-26,2026,April,New Sale\n";
                    headers += "C-1002,Dhaka,Dhaka North,Uttara,Foton,TM3,1,2025-26,2026,April,Resale\n";
                    filename = "ACI_System_Sales_Template.csv";
                } else if (type === 'last_year_sales') {
                    headers = "Customer_ID,District,Territory_Name,Upazila,Brand,Model,Unit_Qty,FY,Sales_Year,Sales_Month,Sale_Type\n";
                    headers += "C-LY1001,Dhaka,Dhaka North,Mirpur,Foton,TM3,1,2024-25,2025,April,New Sale\n";
                    headers += "C-LY1002,Dhaka,Dhaka North,Uttara,Foton,TM3,1,2024-25,2025,April,Resale\n";
                    ...
                    headers = "FY,Month,Territory_Name,Perfile_OD,Total_Overdue\n";
                    headers += "2025-26,April,Dhaka North,4500,1250000\n";
                                ...
                                setTimeout(async () => {
                                    try {
                                        ...
                                        if (dataKey === 'targets') {
                                            ...
                                            DB.sales = DB.sales.filter(s => s.fy !== '2025-26');
                                            if (app.neonSQL) await app.neonSQL`DELETE FROM sales WHERE fy = '2025-26'`;
                                        } else if (dataKey === 'last_year_sales') {
                                            DB.sales = DB.sales.filter(s => s.fy !== '2024-25');
                                            if (app.neonSQL) await app.neonSQL`DELETE FROM sales WHERE fy = '2024-25'`;
                                        ...
                                        parsedData.forEach((row, idx) => {
                                            ...
                                            if (dataKey === 'targets') {
                                                ...
                                                const record = {
                                                    ...
                                                    fy: app.normalizeFY(app.getCSVValue(row, ['FY', 'Fiscal_Year', 'FiscalYear', 'Year']), recordMonth, '2025-26'),
                                                    month: recordMonth,
                                                    territory_id: terrId,
                                                ...
                                                const record = {
                                                    ...
                                                    delivery_date: app.getCSVValue(row, ['Delivery_Date', 'DeliveryDate', 'Delivery']) || '2026-04-01',
                                                    first_inst_date: app.getCSVValue(row, ['First_Inst_Date', 'FirstInstDate']) || '2026-05-01',
                                                ...
                                                const salesYear = parseInt(app.getCSVValue(row, ['Sales_Year', 'SalesYear', 'Year'])) || 2026;
                                                ...
                                                const record = {
                                                    ...
                                                    fy: app.normalizeFY(app.getCSVValue(row, ['FY', 'Fiscal_Year', 'FiscalYear', 'Year']), recordMonth, (dataKey === 'sales' ? '2025-26' : '2024-25')),
                                                    sales_year: parseInt(app.getCSVValue(row, ['Sales_Year', 'SalesYear', 'Year'])) || 2026,
                                        ...
                                        if (app.neonSQL && newRecords.length > 0) {
                                            ...
                                            for(let i=0; i<newRecords.length; i+=batchSize) {
                                                ...
                                                const promises = chunk.map(r => {
                                                     if(dataKey === 'targets') return app.neonSQL`INSERT INTO targets (id, fy, month, territory_id, upazila, brand, sale_type, target_qty) VALUES (${r.id}, ${r.fy}, ${r.month}, ${r.territory_id}, ${r.upazila}, ${r.brand}, ${r.sale_type}, ${r.target_qty})`;
            ...
            saveLink: (e) => {
                ...
                DB.links.push({
                    id: 'lnk_' + Math.floor(Math.random() * 10000),
                    title, url, type, icon
                });
            ...
            renderTIVReporting: () => {
                ...
                const prevMonth = currentMonthIdx > 0 ? months[currentMonthIdx - 1] : 'December';
                            ...
                            <table class="w-full text-left text-sm whitespace-nowrap">
                                <thead class="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase text-xs tracking-wider">
                                    <tr>
                                        <th class="px-6 py-4 font-semibold">Territory</th>
                                        <th class="px-6 py-4 font-semibold">Status</th>
                                        <th class="px-6 py-4 font-semibold text-right">Total Volume</th>
                                        <th class="px-6 py-4 font-semibold text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                        ...
                        return `
                                            <tr class="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td class="px-6 py-4">
                                                    ...
                                                    ` : `
                                                        <span class="bg-amber-50 text-amber-600 px-2 py-1 rounded-full text-[10px] font-bold flex items-center gap-1 w-max border border-amber-100">
            ...
            deleteTIVModel: (brandName, modelName) => {
                const brandObj = DB.tiv_brands.find(b => b.name === brandName);
                if (brandObj) {
            ...
            renderSystemSettings: () => {
                ...
                document.querySelectorAll('#sidebar-nav button').forEach(btn => {
                    ...
                    if (span && span.innerText === 'System Settings') {
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div class="md:col-span-2 space-y-6">
                                ...
                                <div class="glass p-6 rounded-2xl shadow-md border border-white/60 relative overflow-hidden">
                                    ...
                                    <div class="flex items-center gap-2.5 bg-slate-50 border border-slate-100 p-3.5 rounded-xl mb-6 shadow-inner">
            ...
            saveSystemSettings: () => {
                ...
                app.showLoader("Saving system month configurations...");

                setTimeout(() => {
                    app.currentMonth = currentMonth;
                    app.lastMonth = lastMonth;

                    localStorage.setItem('aci_current_month', currentMonth);
                    localStorage.setItem('aci_last_month', lastMonth);

                    app.soMonthTab = null;

                    app.hideLoader();
                    app.showToast(`System Settings Saved: ${currentMonth} / ${lastMonth}`, "success");
            ...
            navigateSO: (view) => {
                ...
                else if (view === 'profile') app.renderSOProfile();
            ...
            renderSODashboard: () => {
                ...
                const currentFY = '2025-26'; // Fixed to current active FY
                ...
                const yearlyTargets = DB.targets.filter(t => t.territory_id === terrId && t.brand === brand && t.fy === currentFY && t.sale_type === saleType);
                ...
                const currentSply = DB.sales.filter(s => s.territory_id === terrId && s.brand === brand && s.sale_type === saleType && s.sales_month === currentMonth && s.fy === '2024-25').reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);
                ...
                // Pending Manual Deliveries specifically for this SO and selected brand/type
                const myManualDeliveries = brandSales.filter(s => s.is_manual);
                ...
                const tillLastMonthSales = brandSales.filter(s => s.fy === currentFY && ytdMonths.includes(s.sales_month));
                const tillLastMonthSalesUnits = tillLastMonthSales.reduce((sum, s) => sum + Number(s.unit_qty || 0), 0);
                ...
                // Basic mock fallback for older months without explicit mock entries:
                const selMonthSply = selMonth === app.currentMonth ? (DB.sales.filter(s => s.territory_id === terrId && s.brand === brand && s.sale_type === saleType && s.sales_month === app.currentMonth && s.fy === '2024-25').reduce((sum, s) => sum + Number(s.unit_qty || 0), 0) || Math.floor(monthlyBudget * 0.9)) : (selMonth === app.lastMonth ? mockSply.sply : Math.floor(monthlyBudget * 0.8));
                ...
                const soEmiFirstTwo = DB.emi.filter(e => e.territory_id === terrId && (e.installment_no === 1 || e.installment_no === 2));
                const totalFirstTwoCust = soEmiFirstTwo.length;
                const unpaidFirstTwoCust = soEmiFirstTwo.filter(e => e.collected < e.installment).length;
                    ...
                    <div class="pb-4 fade-in">
                        <div class="flex justify-between items-center mb-6">
                            <div class="flex items-baseline gap-1.5">
                                ...
                                <h2 class="text-base font-bold text-slate-800 tracking-tight">${app.currentUser.name.split(' ')[0]}</h2>
                            </div>
                        ...
                        <div class="glass p-4 rounded-xl shadow-sm border border-slate-100 mb-4 relative overflow-hidden">
                            ...
                            <div class="grid grid-cols-6 text-center divide-x divide-slate-100">
                                <div class="px-1 flex flex-col justify-center">
                                    <p class="text-[9px] text-slate-400 uppercase font-semibold">YTD Target</p>
                                    <p class="font-bold text-slate-800 text-sm">${ytdTargetTillLastMonth}</p>
                                    <p class="text-[8px] text-slate-400 font-semibold mt-0.5">FY: ${totalYearlyTarget}</p>
                                </div>
                                <div class="px-1 flex flex-col justify-center">
                                    <p class="text-[9px] text-slate-400 uppercase font-semibold">Sales</p>
                                    <p class="font-bold text-aci-blue text-sm">${tillLastMonthSalesUnits}</p>
                        ...
                        <div class="bg-gradient-to-br ${brand === 'Foton' ? 'from-foton to-[#03133d] shadow-foton/20' : 'from-mahindra to-[#b81b31] shadow-mahindra/20'} rounded-2xl p-4 mb-4 relative overflow-hidden shadow-lg text-white">
                            ...
                            <div class="mt-3 relative z-10 cursor-pointer group" onclick="app.renderSOEMI()">
                                <div class="bg-gradient-to-r from-indigo-500/30 to-purple-500/30 border border-white/20 rounded-xl p-3 flex justify-between items-center hover:bg-white/20 transition-all backdrop-blur-sm shadow-inner group-hover:shadow-lg">
                                    <div class="flex items-center gap-2.5">
                                        <div class="bg-white/10 p-2 rounded-lg text-indigo-200 group-hover:scale-110 group-hover:text-white transition-all shadow-sm">
                                            <i data-lucide="calendar-clock" class="w-4 h-4"></i>
                                        </div>
                                        <div>
                                            <p class="text-[9px] text-white/90 font-bold uppercase tracking-widest mb-0.5 flex items-center gap-1">1st & 2nd EMI <span class="bg-rose-500/80 text-white text-[7px] px-1 rounded animate-pulse">ACTION</span></p>
                                            <div class="flex items-baseline gap-2">
                                                <span class="text-sm font-black text-indigo-100">${totalFirstTwoCust} <span class="text-[9px] font-medium text-white/60">Total</span></span>
                                                <span class="text-[10px] text-white/30">|</span>
                                                <span class="text-sm font-black text-rose-300">${unpaidFirstTwoCust} <span class="text-[9px] font-medium text-white/60 uppercase">Unpaid</span></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text-indigo-200 group-hover:translate-x-1.5 group-hover:text-white transition-all">
                        ...
                        <!-- Pending Manual Deliveries List (Modern Table Style) -->
                        ${myManualDeliveries.length > 0 ? `
                            <div class="mb-6">
                                <h3 class="font-bold text-slate-800 text-sm mb-3 flex items-center gap-2">
                                    ...
                                    Manual Field Deliveries
                                </h3>
                                <div class="glass overflow-hidden rounded-2xl border border-white/40 shadow-xl overflow-x-auto no-scrollbar">
                                    <table class="w-full text-left text-[11px] whitespace-nowrap">
                                        <thead class="${brand === 'Foton' ? 'bg-foton' : 'bg-mahindra'} text-white/90 uppercase tracking-[0.1em] text-[9px] font-black border-b border-white/20">
                                            <tr>
                                                ...
                                                <tr class="hover:bg-white/20 transition-colors group">
                                                    ...
                                                    <td class="px-4 py-3 text-right">
                                                        <div class="flex items-center justify-end gap-1.5">
                                                            ...
                                                            <span class="text-[10px] font-black text-indigo-600 uppercase tracking-tight">Manual Sync Required</span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            `).join('')}
                        ...
                        <!-- Upazila Wise Summary (YTD / This FY) -->
                        <div class="bg-white rounded-xl shadow-sm border border-slate-100 mb-2 overflow-hidden">
                            <div class="p-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <i data-lucide="map" class="w-4 h-4 text-aci-blue"></i>
                                    <h3 class="font-bold text-slate-800 text-sm">Upazila Wise Breakdown (This FY)</h3>
                                </div>
                                <span class="bg-aci-blue/10 text-aci-blue text-[9px] font-bold px-2 py-0.5 rounded tracking-wider uppercase">Till Now</span>
                            </div>
                            <div class="divide-y divide-slate-100">
                                ${Object.keys(upaStats).map(k => ({ name: k, sales: upaStats[k].ytdSales }))
                        .sort((a, b) => b.sales - a.sales)
                        .map(u => {
                            const isZero = u.sales === 0;
                            return `
                                    <div class="py-2.5 px-3 transition-colors flex justify-between items-center ${isZero ? 'bg-rose-50/50 hover:bg-rose-50' : 'hover:bg-slate-50'}">
                                        <div class="flex items-center gap-2.5">
                                            ${isZero ? '<i data-lucide="alert-circle" class="w-3.5 h-3.5 text-red-500 animate-pulse"></i>' : '<div class="w-1.5 h-1.5 rounded-full bg-slate-300"></div>'}
                                            <h4 class="font-bold text-[13px] ${isZero ? 'text-red-600' : 'text-slate-600'}">${u.name}</h4>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            ${isZero ? '<span class="bg-red-100 text-red-600 text-[8px] font-black uppercase px-2 py-0.5 rounded border border-red-200 tracking-widest hidden sm:inline-block">Zero</span>' : ''}
                                            <p class="text-sm font-black w-6 text-right ${isZero ? 'text-red-500' : 'text-slate-800'}">${u.sales}</p>
                                        </div>
                                    </div>
                                    `;
                        }).join('')}
                        ...
                        <!-- SO ACTIONABLE INTELLIGENCE SECTION -->
                        ${(() => {
                        ...
                        return `
                            <div class="mt-8 mb-6 fade-in">
                                <div class="flex items-center gap-3 mb-4 pl-1">
                                    <div class="p-2 bg-blue-100 rounded-xl border border-blue-200 shadow-inner"><i data-lucide="bar-chart-3" class="w-5 h-5 text-blue-700"></i></div>
                                    <div>
                                        <h3 class="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-indigo-800 tracking-tight">Actionable Territory Intelligence</h3>
                                        <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">Shortfall & Practical Recovery Data</p>
                                    </div>
                                </div>
                                
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    
                                    <!-- Left Column: Critical Issues & Recovery -->
                                    <div class="flex flex-col gap-4">
                                        <!-- Zero Sales Zones (FY) -->
                                        <div class="bg-gradient-to-br from-rose-50 to-red-50 rounded-2xl p-5 shadow-sm border border-red-100 relative overflow-hidden">
                                            <div class="absolute -right-4 -top-4 opacity-5"><i data-lucide="alert-octagon" class="w-32 h-32 text-red-600"></i></div>
                                            <div class="flex items-center gap-2 mb-3 relative z-10">
                                                <span class="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                                                <h4 class="text-red-800 text-xs font-extrabold uppercase tracking-wider">Critical: Zero Sales Zones</h4>
                                            </div>
                                            <p class="text-slate-700 text-sm leading-relaxed relative z-10 mb-4">
                                                The following areas have <strong class="text-red-600 font-bold">0 units</strong> sold <strong class="underline decoration-red-300">this entire fiscal year</strong> against active budgets. Immediate pipeline reviews required:
                                            </p>
                                            <div class="flex flex-wrap gap-2 relative z-10">
                                                ${zeroSalesUpazilas.length > 0
                                ? zeroSalesUpazilas.map(u => `<span class="bg-white text-red-600 border border-red-200 px-2.5 py-1 rounded-md text-xs font-bold shadow-sm">${u}</span>`).join('')
                                : `<span class="text-green-700 bg-green-100 border border-green-200 px-3 py-1 rounded-md text-xs font-bold flex items-center gap-1.5"><i data-lucide="check-circle-2" class="w-4 h-4"></i> All areas successfully activated this FY!</span>`}
                                            </div>
                                        </div>

                                        <!-- Shortfall Target Plan -->
                                        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 relative overflow-hidden flex flex-col justify-center">
                                            <h4 class="text-slate-800 text-sm font-bold flex items-center gap-2 mb-4 relative z-10"><i data-lucide="target" class="w-4 h-4 text-indigo-500"></i> Target Plan</h4>
                                            
                                            <div class="grid grid-cols-3 gap-2 text-center divide-x divide-slate-100 mb-4 relative z-10">
                                                <div>
                                                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Budget</p>
                                                    <p class="text-xl font-black text-slate-800">${monthlyBudget}</p>
                                                </div>
                                                <div>
                                                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Sold</p>
                                                    <p class="text-xl font-black text-aci-blue">${currentSalesUnits}</p>
                                                </div>
                                                <div>
                                                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Shortfall</p>
                                                    <p class="text-xl font-black text-amber-600">${remainingTarget}</p>
                                                </div>
                                            </div>
                                            
                                            <div class="bg-slate-50 border border-slate-200 rounded-xl p-3 relative z-10 flex items-center justify-between">
                                                <div>
                                                    <p class="text-xs text-slate-800 font-bold">Required Weekly Run-Rate</p>
                                                    <p class="text-[10px] text-slate-500 font-medium mt-0.5">To close gap in remaining ${daysLeft} days</p>
                                                </div>
                                                <div class="text-right">
                                                    <span class="text-xl font-black text-indigo-600">${requiredRunRate}</span>
                                                    <span class="text-xs text-indigo-400 font-bold"> units/wk</span>
                                                </div>
                                            </div>
                                            <p class="text-[10px] text-center font-semibold mt-3 ${splyGrowth >= 0 ? 'text-green-600' : 'text-red-500'}">
                                                * Currently tracking ${splyGrowth >= 0 ? '+' : ''}${splyGrowth}% vs SPLY trajectory.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-4">
                                        <!-- AI Month-End Predictor -->
                                        <div class="glass p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-center">
                                            <div class="absolute -right-10 -bottom-10 bg-purple-500/10 w-32 h-32 rounded-full blur-xl pointer-events-none"></div>
                                            <h4 class="text-slate-800 text-sm font-bold flex items-center gap-2 mb-4 relative z-10"><i data-lucide="brain-circuit" class="w-4 h-4 text-purple-500"></i> AI Month-End Predictor</h4>
                                            
                                            <p class="text-xs text-slate-600 mb-4 relative z-10">Based on your current daily velocity (Day ${currentDay} of ${daysInMonth}), the system forecasts your final closing:</p>
                                            
                                            <div class="flex items-center justify-between mb-2 relative z-10">
                                                <div class="text-center bg-slate-50 p-2 rounded-lg border border-slate-100 flex-1 mr-2">
                                                    <p class="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Projected Closing</p>
                                                    <p class="text-2xl font-black ${isPacingWell ? 'text-green-600' : 'text-slate-800'}">${predictedClose}</p>
                                                </div>
                                                <div class="text-center ${isPacingWell ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'} p-2 rounded-lg border flex-1 ml-2">
                                                    <p class="text-[10px] ${isPacingWell ? 'text-green-600' : 'text-red-500'} uppercase font-bold tracking-wider">${isPacingWell ? 'Surplus' : 'Predicted Miss'}</p>
                                                    <p class="text-2xl font-black ${isPacingWell ? 'text-green-700' : 'text-red-600'}">${isPacingWell ? '+' + (predictedClose - monthlyBudget) : predictedShortfall}</p>
                                                </div>
                                            </div>
                                            
                                            <div class="w-full bg-slate-100 rounded-full h-1.5 mt-2 overflow-hidden relative z-10">
                                                <div class="${isPacingWell ? 'bg-green-500' : 'bg-amber-500'} h-1.5 rounded-full" style="width: ${Math.min(100, (predictedClose / monthlyBudget) * 100)}%"></div>
                                            </div>
                                        </div>

                                        <!-- AI Next Best Action -->
                                        <div class="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-2xl p-5 shadow-sm border border-indigo-100 relative overflow-hidden flex-1 flex flex-col justify-center">
                                            <h4 class="text-indigo-800 text-sm font-bold flex items-center gap-2 mb-3 relative z-10"><i data-lucide="lightbulb" class="w-4 h-4 text-amber-500 fill-amber-500"></i> Next Best Action</h4>
                                            <p class="text-slate-700 text-sm leading-relaxed relative z-10">
                                                ${topGapUpazila
                                ? `To correct the predicted shortfall, divert primary field efforts to <strong class="text-indigo-700 font-black uppercase text-base">${topGapUpazila}</strong>. It holds the highest localized gap (<span class="font-bold text-red-500">${maxMonthGap} unfulfilled units</span>) against the micro-budget.`
                                : `<span class="text-green-700 font-medium">Your territory is fully optimized across all micro-budgets. Continue current trajectory.</span>`
                            }
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <form id="add-delivery-form" onsubmit="app.saveManualDelivery(event)" class="space-y-5 pb-4 relative z-10">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative group">
                                        <select id="del-upazila" class="w-full border-2 border-slate-100 rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:border-aci-blue bg-slate-50/50 appearance-none pr-10" required>
            ...
            purposeSuggestions: [
                ...
                "Water bottle", "Agriculture", "Personal Use", "Public Transport"
            ],
            ...
            saveManualDelivery: (e) => {
                ...
                const upazila = document.getElementById('del-upazila').value;
                ...
                setTimeout(() => {
                    ...
                    DB.sales.push({
                        ...
                        sales_month: 'April',
                        sale_type: type,
                        financials: { tp, dp, tenure },
                        discounts: { type: discType, amount: discAmt, gift: giftItem },
                    ...
                    // Re-render dashboard to show immediate reflection
            ...
            renderSOEMI: () => {
                ...
                const totalInstallment = soEmi.reduce((sum, e) => sum + e.installment, 0);
                const totalCollected = soEmi.reduce((sum, e) => sum + e.collected, 0);
                ...
                const paidCustomers = soEmi.filter(e => e.collected >= e.installment).length;
                    ...
                    <div class="pb-6 fade-in">
                        ...
                        <div class="flex gap-2 mb-5">
                            <div class="relative flex-1">
                                ...
                                <input type="text" id="emi-search" placeholder="Search customer or code..." class="w-full bg-white border border-slate-200 text-sm rounded-lg pl-9 pr-4 py-2.5 focus:outline-none focus:border-aci-blue shadow-sm" onkeyup="app.filterEMI()">
                            </div>
                        ...
                        <!-- Customer Cards -->
                        <div class="space-y-4" id="emi-list">
            ...
            generateEMICardHTML: (e, serial) => {
                const hasOverdue = e.overdue_total > 0;
                const isInstCleared = e.collected >= e.installment;
                const totalDue = e.installment + e.overdue_total;
                const isFullyCleared = e.collected >= totalDue && totalDue > 0;
                ...
                if (e.installment_no === 1) {
                    instBadge = `<span class="bg-indigo-100 text-indigo-700 text-[9px] font-bold px-1.5 py-0.5 rounded ml-2 border border-indigo-200">Inst. #1</span>`;
                } else if (e.installment_no === 2) {
                    instBadge = `<span class="bg-purple-100 text-purple-700 text-[9px] font-bold px-1.5 py-0.5 rounded ml-2 border border-purple-200">Inst. #2</span>`;
                } else if (e.installment_no) {
                    instBadge = `<span class="bg-slate-100 text-slate-600 text-[9px] font-bold px-1.5 py-0.5 rounded ml-2 border border-slate-200">Inst. #${e.installment_no}</span>`;
                }

                return `
                    <div class="emi-card relative bg-white p-4 rounded-xl shadow-sm border ${cardCleared ? 'border-green-200 bg-green-50/40' : 'border-slate-200'} transition-all overflow-hidden" data-customer="${e.customer.toLowerCase()}" data-code="${(e.customer_code || '').toLowerCase()}" data-status="${cardCleared ? 'paid' : 'unpaid'}">
                        
                        <!-- Fully Cleared Background Watermark -->
                        ${isFullyCleared ? '<div class="absolute -right-6 -top-6 opacity-10 pointer-events-none"><i data-lucide="check-circle" class="w-32 h-32 text-green-600"></i></div>' : ''}

                        <div class="flex justify-between items-start mb-2 relative z-10">
                            <div class="flex items-start gap-2">
                                <span class="text-sm font-bold text-slate-400 mt-0.5">${serial}.</span>
                                <div>
                                    <h4 class="font-bold text-slate-800 text-sm flex items-center flex-wrap gap-y-1">
                                        ${e.customer}
                                        ${instBadge}
                                        ${isFullyCleared ? '<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-[9px] font-bold ml-2 flex items-center gap-1"><i data-lucide="check-circle-2" class="w-3 h-3"></i> Fully Paid</span>' : ''}
                                    </h4>
                                    <p class="text-sm font-bold text-aci-blue mt-0.5">${e.customer_code || 'N/A'}</p>
                                </div>
                            </div>
                        </div>
                        
                        <div class="flex gap-4 mb-3 pb-3 border-b border-slate-100 mt-3 relative z-10">
                            <div class="flex-1">
                                <p class="text-[9px] text-slate-400 uppercase font-semibold">Delivery Date</p>
                                <p class="text-[11px] font-semibold text-slate-700">${e.delivery_date || 'N/A'}</p>
                            </div>
                            <div class="flex-1 text-right">
                                <p class="text-[9px] text-slate-400 uppercase font-semibold">1st Inst. Date</p>
                                <p class="text-[11px] font-semibold text-slate-700">${e.first_inst_date || 'N/A'}</p>
                            </div>
                        </div>

                        <div class="flex gap-4 mb-3 pb-3 border-b border-slate-100 mt-2 relative z-10">
                            <div class="flex-1">
                                <p class="text-[10px] text-slate-400 uppercase">Brand</p>
                                <p class="text-xs font-semibold text-slate-700">${e.brand || 'N/A'}</p>
                            </div>
                            <div class="flex-1 text-right">
                                <p class="text-[10px] text-slate-400 uppercase flex items-center justify-end gap-1">
                                    Total Overdue ${hasOverdue ? '<i data-lucide="alert-triangle" class="w-3 h-3 text-red-500"></i>' : ''}
                                </p>
                                <div class="flex items-center justify-end gap-2 mt-0.5">
                                    <p class="text-xs font-semibold ${hasOverdue ? 'text-red-600' : 'text-slate-700'}">${app.formatCurrency(e.overdue_total)}</p>
                                    ${hasOverdue ? `<span class="bg-red-50 text-red-600 text-[9px] font-bold px-1.5 py-0.5 rounded-md border border-red-100">${e.overdue_count} Overdue</span>` : ''}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-3 relative z-10">
                            <div>
                                <p class="text-[10px] text-slate-400 uppercase">Installment</p>
                                <div class="flex items-center gap-1.5 mt-0.5">
                                    <p class="text-sm font-bold ${isInstCleared ? 'text-green-600' : 'text-slate-800'}">${app.formatCurrency(e.installment)}</p>
                                    ${isInstCleared ? '<i data-lucide="check-circle" class="w-5 h-5 text-green-500 drop-shadow-sm"></i>' : ''}
                                </div>
                            </div>
                            ...
                            <div class="flex-1 flex flex-col items-end gap-1">
                                <div class="text-[9px] text-slate-500 font-semibold uppercase flex flex-col items-end gap-0.5">
                                    <div class="flex items-center gap-1">
                                        Paid: <span class="${e.collected > 0 ? 'text-green-600 text-lg drop-shadow-sm' : 'text-slate-400 text-sm'} font-extrabold tracking-tight">${app.formatCurrency(e.collected)}</span>
                                    </div>
                                    ${e.collected > totalDue ? `<span class="bg-green-100 text-green-700 border border-green-200 px-1.5 py-0.5 rounded text-[8px] font-bold mt-0.5">Advance: ${app.formatCurrency(e.collected - totalDue)}</span>` : ''}
                                </div>
                                <div class="flex items-center gap-1.5 mt-1">
                                    <div class="relative w-24">
                                        <span class="absolute left-2 top-1/2 transform -translate-y-1/2 text-slate-400 text-sm">৳</span>
                                        <input type="number" id="emi-input-${e.id}"
                                               class="w-full bg-white border ${isFullyCleared ? 'border-green-300 bg-green-50' : 'border-slate-300 focus:border-aci-blue'} text-sm font-bold text-slate-800 rounded-lg pl-5 pr-2 py-1.5 focus:outline-none transition-colors text-right shadow-sm" 
                                               placeholder="Add">
                                    </div>
                                    <button onclick="app.saveEMICollection('${e.id}')" 
                                            class="${isFullyCleared ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-aci-blue hover:bg-blue-800 text-white'} p-1.5 rounded-lg transition-colors shadow-sm">
            ...
            saveEMICollection: (emiId) => {
                const inputEl = document.getElementById(`emi-input-${emiId}`);
                ...
                if (emiRecord) {
                    emiRecord.collected += val; // Add the new amount to the previously collected amount
                    app.showToast(`Successfully added ${app.formatCurrency(val)} to collection.`, 'success');
                    app.renderSOEMI(); // Re-render to update summary and UI state (also clears input)
                    app.updateSOBadge();
                }
            },
            ...
            renderSOProfile: () => {
                const html = `
                    <div class="text-center pt-8 pb-4">
                        ...
                        <p class="text-sm text-slate-500">${app.currentUser.email}</p>
                        <span class="inline-block mt-2 bg-slate-100 text-slate-600 text-xs px-3 py-1 rounded-full font-medium border border-slate-200">Sales Officer</span>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden mt-6">
                        <div class="px-4 py-3 border-b border-slate-100 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500">${app.currentUser.territories.map(tId => DB.territories.find(t => t.id === tId).name).join(', ')}</p>
                            </div>
                        </div>
                        <div class="px-4 py-3 border-b border-slate-100 flex items-center gap-3">
                    ...
                    <button onclick="app.logout()" class="w-full mt-8 bg-red-50 text-red-600 font-bold py-3 rounded-xl border border-red-100 flex items-center justify-center gap-2 active:bg-red-100 transition-colors">
            ...
            // Used by both SO and AM portals (scoped to assigned territories).
            // ==========================================
            _renderPulseMatrixHTML: (assignedTerritoryIds) => {
                ...
                const rowsHTML = territories.map((t, idx) => {
                    ...
                    fiscalMonths.forEach(m => {
                        ...
                        const mBudget = mTgts.length > 0 ? mTgts.reduce((s, tg) => s + tg.target_qty, 0) : Math.round(totalFYBudget / 12);
                    ...
                    Object.entries(quarters).forEach(([qName, qMs]) => {
                        const qBudget = qMs.reduce((s, m) => s + monthlyPerf[m].budget, 0);
                        const qSales  = qMs.reduce((s, m) => s + monthlyPerf[m].sales,  0);
                        const qSply   = qMs.reduce((s, m) => s + monthlyPerf[m].sply,   0);
                ...
                <div class="pb-6 fade-in">
                    <div class="flex items-center justify-between mb-5">
                        <div>
                            ...
                            <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5 ml-10">12-Month Territory Breakdown &middot; FY 2025-26</p>
                        </div>
                    </div>
            ...
            // ==========================================
            // SO: Performance Matrix (assigned territories only)
            // ==========================================
            renderSOPulseMatrix: () => {
